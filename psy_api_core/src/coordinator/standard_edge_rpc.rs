...
pub trait CoordinatorEdgeRpc<F, Hash, JobId, ZKProof>: NodeEdgeWorkerRpcServer<Hash, JobId> {
    ...
    async fn submit_guta(&self, input: GlobalUserTreeAggregatorHeaderWithTagValueAndJobType<F, Hash>, proof: Vec<u8>, realm_id: u64, proposal: Option<Vec<u8>>, certificate: Option<Vec<u8>>, finalize_binding: Vec<u8>) -> RpcResult<String>;
