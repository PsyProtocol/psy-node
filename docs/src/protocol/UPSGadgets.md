...
This protocol reference covers local User Proving Session gadgets and circuit wrappers within the [gadget index](Gadgets.md).
...
- [14. UPS Circuits](#14-ups-circuits)
- [15. Empty stub note](#15-empty-stub-note)
...
These circuit wrappers compose the gadgets above and register tree-aware public inputs. Core LOC excludes blanks/comments/`#[cfg(test)]`.

### `UPSStartSessionCircuit`

-   **File:** `client_prover/psy_circuit/psy_network_circuit/src/ups/circuits/ups_start.rs` (core ~108)
-   **Gadgets:** `UPSStartStepGadget`
-   **Public Inputs:** `tree_aware(empty_proof_tree_root, header.hash)`
-   **Constraints (pseudocode):**

    ```text
    start = UPSStartStepGadget
    PI = tree_aware(EMPTY_UPS_PROOF_TREE_ROOT, start.header.to_hash())
    ```

-   **Role:** First UPS leaf for an already-registered user.

### `UPSStartSessionRegisterUserCircuit`

-   **File:** `…/ups/circuits/ups_start_register_user.rs` (core ~113)
-   **Gadgets:** `UPSStartStepRegisterUserGadget`
-   **Public Inputs:** Same tree-aware form; **separate** UPS whitelist fingerprint from registered-user start.
-   **Constraints:** As register-user gadget; empty GUSR leaf + registration-tree membership.
-   **Role:** First UPS leaf for a newly registered user.

### `UPSCFCStandardTransactionCircuit`

-   **File:** `…/ups/circuits/ups_cfc_standard.rs` (core ~115)
-   **Gadgets:** `VerifyPreviousUPSStepProofInProofTreeGadget` + `UPSVerifyCFCStandardStepGadget`
-   **Public Inputs:** `tree_aware(current_proof_tree_root, new_header.hash)`
-   **Constraints (pseudocode):**

    ```text
    prev = VerifyPreviousUPSStep(…); root = prev.current_proof_tree_root
    step = UPSVerifyCFCStandardStep(prev.header, root)
    PI = tree_aware(root, step.new_header.to_hash())
    ```

-   **Role:** One standard CFC step in the UPS proof tree.

### `UPSCFCDeferredTransactionCircuit`

-   **File:** `…/ups/circuits/ups_cfc_deferred_tx.rs` (core ~123)
-   **Gadgets:** Previous-step verify + `UPSVerifyPopDeferredTxStepGadget`
-   **Public Inputs:** Same tree-aware form over the new header.
-   **Role:** Pop/apply one deferred debt transaction step.
...
-   **File:** `…/ups/gadgets/ups_new_user_start.rs` is an **empty** source file in this worktree (0 core LOC). Do not treat it as a live gadget. Register-user start is `UPSStartStepRegisterUserGadget` / `UPSStartSessionRegisterUserCircuit`.
