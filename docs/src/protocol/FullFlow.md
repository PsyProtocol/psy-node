...
> **Currency note (2026-09-08):** Realm root proving uses `RealmFinalizeGUTA` (63) then P2P Proposal/Certificate. No in-circuit `WrappedSignatureProof` (64) after BLS auth cutover ([ProvingJobs.md](./ProvingJobs.md) §2). Aggregation merge: `DualVariableHeightStateTransitionGadget` (not TwoNCA). Live GUTA names: [GUTAV2Circuits.md](./GUTAV2Circuits.md).
...
*   **User Registration Tree:** Tracks registered users and their public keys (updated by coordinator `BatchAppendUserRegistrationTree`, not a GUTA-named register circuit).
...
Realm workers aggregate End Cap proofs into GUTA headers, then finalize with `RealmFinalizeGUTA` (63). Live circuit names, public inputs, and constraint pseudocode: [GUTAV2Circuits.md](./GUTAV2Circuits.md), [ProvingJobs.md](./ProvingJobs.md), `AUDIT_SCOPE.md` §1.7.

### 5.1. EndCap admission (`GUTAVerifySingleEndCapCircuitV2` / `TwoEndCap`)

*   **Core Gadget:** `VerifyEndCapProofGadget` + SVH / DVH
*   **What it Proves:** End Cap validity + known fingerprint; historical checkpoint membership; GUSR leaf transition(s) under a shared whitelist.
*   **Public Inputs:** `H2(header_hash, rewards_tree_value)` (4 felts).
*   **Assumptions carried upward:** GUTA whitelist root; current checkpoint root from historical proof.

### 5.2. User registration (coordinator)

*   **Live circuit:** `BatchAppendUserRegistrationTree` via `BatchAppendUserRegistrationTreeGadget` / Spiderman appends on URT.
*   See [CoordinatorGadgets.md](./CoordinatorGadgets.md).

### 5.3. GUTA aggregation (`GUTAVerifyTwoGUTACircuitV2` and siblings)

*   **Core Gadgets:** `VerifyGUTAProofGadget`, `DualVariableHeightStateTransitionGadget` (replaces TwoNCA), linear / LLRV / line gadgets, checkpoint-upgrade historical proofs.
*   **What it Proves:** Child proofs valid and whitelisted; same checkpoint context (or upgraded); combined state transition and stats.
*   **Contribution to Horizontal Scalability:** Parallel MapReduce-style merge across realms.

### 5.4. No-change and realm finalize

*   **`GUTANoChangeGadget`:** Acknowledge checkpoint advancement with a no-op GUSR transition ([RealmGUTAGadgets.md](./RealmGUTAGadgets.md) §8).
*   **`RealmFinalizeGUTACircuit` (63):** Finalize realm root with rotation / validator / DA-fee constraints; submit with P2P Proposal+Certificate ([GUTAV2Circuits.md](./GUTAV2Circuits.md) §5).
