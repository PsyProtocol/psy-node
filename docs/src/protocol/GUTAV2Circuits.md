...
> Currency: `TwoNCAStateTransitionGadget` is obsolete — use `DualVariableHeightStateTransitionGadget`. Circuit type IDs: `docs/src/protocol/ProvingJobs.md`.
...
*   **Constraints (pseudocode):**
    ...
    ```
*   **Role:** Realm root submitted with P2P Proposal+Certificate to Coordinator `psy_submit_guta`. See `docs/src/dev/reward-tree-circuits.md` and `realm-finalize-bls-auth.md`.
