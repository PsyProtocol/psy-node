...
This protocol walkthrough tracks proofs and assumptions from local user execution through realm and coordinator aggregation, expanding the [circuit index](Circuits.md).

> **Currency note (2026-09-08):** Realm root proving uses `RealmFinalizeGUTA` (63) then P2P Proposal/Certificate. No in-circuit `WrappedSignatureProof` (64) after BLS auth cutover ([ProvingJobs.md](./ProvingJobs.md) §2). Aggregation merge: `DualVariableHeightStateTransitionGadget` (not TwoNCA / NCA-proof gadget).

> Updated: 2026-09-08.

## Abstract

This document traces the zero-knowledge proof flow from local user execution through Realm and Coordinator aggregation to the final block proof, recording the assumptions discharged at each stage.
...
- [1. User Proving Session: Local Execution](#1-user-proving-session-local-execution)
- [2. Network Aggregation](#2-network-aggregation-parallel-realm-and-coordinator-execution)
- [3. Coordinator Aggregation](#3-coordinator-aggregation)
- [4. Final Block Proof](#4-final-block-proof)
...
## 2. Network Aggregation: Parallel Realm and Coordinator Execution
...
-   **Proves:**
    ...
    -   The End Cap proof(s) were generated by the designated End Cap circuit (fingerprint checked against `known_end_cap_fingerprint`).
    ...
    -   *(For TwoEndCap):* Dual variable-height merge correctly combines the two individual user state transitions (`start_leaf` -> `end_leaf` at user indices) into a single state transition at their parent node in the `GUSR` tree. Statistics are correctly summed.
...
-   **Circuit(s):**
    ...
    -   *(Enum-only / not in `cached_circuit_library`:)* `GUTALeftEndCapRightGUTA` — no live circuit file in this worktree; planner uses LeftGUTA+RightEndCap + TwoGUTA / linear / LLRV instead.
-   **High-Level Purpose:** To recursively combine verified state transitions within the GUTA hierarchy. These circuits take proofs representing changes in subtrees (either previous GUTA aggregations or newly processed End Caps) and merge them into a proof for the parent node, typically using DualVariableHeight / linear / LLRV transitions.
-   **Proves:**
    ...
    -   The merge logic (`DualVariableHeightStateTransitionGadget`) correctly combines the state transitions (`SubTreeNodeStateTransition` from input GUTA headers or derived from EndCap results) via DualVariableHeight state transition.
    -   Statistics are correctly summed (`GUTAStatsGadget.combine_with`).
    -   Outputs a `GlobalUserTreeAggregatorHeader` for the parent DVH / linear / LLRV node.
-   **How:**
    -   `VerifyGUTAProofGadget` (for GUTA inputs).
    -   `VerifyEndCapProofGadget` (for EndCap inputs).
    -   `DualVariableHeightStateTransitionGadget`: Core aggregation logic using dual variable-height delta proofs.
    -   Connections ensuring consistency of `checkpoint_tree_root` and `guta_circuit_whitelist` between inputs.
-   **Assumes:**
    -   `[A5.1]` Witness data (input proofs, headers/results, whitelist proofs, DVH / linear / LLRV delta proofs) correct initially.
