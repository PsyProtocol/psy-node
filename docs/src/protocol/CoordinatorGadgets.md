...
This protocol reference covers coordinator gadgets and circuit wrappers for registration, deployment, aggregation, and checkpoint updates within the [gadget index](Gadgets.md).
...
- [9. Recursive CST verify gadget](#9-verifyrecursivecheckpointstatetransitionproofgadget)
- [10. Coordinator Circuits](#10-coordinator-circuits)
...
-   **File:** `psy_plonky2_circuits/src/coordinator/gadgets/deploy_contract.rs:17-71` (core lines: 57). **Live layout-aware gadget:** `deploy_contract_v2.rs` `BatchDeployContractsGadget` (core ~402) used by `batch_deploy_contract_v2.rs` circuit.
...
## 9. `VerifyRecursiveCheckpointStateTransitionProofGadget`

-   **File:** `psy_plonky2_circuits/src/coordinator/gadgets/recursive_checkpoint_state_transition_verify.rs` (core ~79 excl. tests)
-   **Purpose:** Inside CST: verify the **previous** checkpoint state-transition proof (genesis fingerprint when `checkpoint_id == 1`, else current CST VK) and bind opaque chain / leaf / root PI fields used by cumulative-chain mode.
-   **Public Inputs:** None alone — wires into `CheckpointStateTransitionCoreGadget` / CST circuit PIs.
-   **Private Inputs / Witness:** Previous CST proof + verifier data; last old leaf/root hashes; previous chain hash; current PI gadget; `checkpoint_id`.
-   **Constraints (pseudocode):**

    ```text
    verify_proof(prev_CST)
    fp = fingerprint(prev_VK)
    if checkpoint_id == 1: fp == known_genesis_fingerprint
    else: fp == current_CST_fingerprint (from witness VK)
    // do not re-derive legacy transition hash; bind chain commitment fields
    ```

-   **Role:** Links CST N to CST N−1 (or genesis) fail-closed.

---

## 10. Coordinator Circuits

Leaf / Part-1 / CST wrappers. Shared leaf PI helper: `compute_agg_state_trackable_final_public_inputs_leaf(whitelist, H2(old,new), worker_tag)` → 4 felts (see [ProvingJobs.md](./ProvingJobs.md) §9).

### `BatchAppendUserRegistrationTreeCircuit` (type 0)

-   **File:** `psy_plonky2_circuits/src/coordinator/circuits/batch_append_user_registration_tree.rs` (core ~191)
-   **Gadgets:** `BatchAppendUserRegistrationTreeGadget`
-   **Constraints:**

    ```text
    g = BatchAppendURT(…)
    ST = H2(g.old_root, g.new_root)
    PI = agg_state_trackable_leaf(whitelist, ST, worker_tag)
    ```

### `BatchDeployContractsCircuit` (type 18)

-   **Files:** `…/circuits/batch_deploy_contract.rs` (core ~192, Spiderman + V1 leaf path); layout-aware **`batch_deploy_contract_v2.rs`** wrapping `deploy_contract_v2::BatchDeployContractsGadget` (gadget core ~402)
-   **Constraints:** Same ST/PI leaf pattern over Spiderman `old_root`/`new_root`; V2 additionally binds typed contract leaves / layout.

### `BatchUpdateContractsCircuit` (type 22)

-   **File:** `…/circuits/batch_update_contract.rs` (core ~219)
-   **Gadgets:** `BatchUpdateContractsGadget`
-   **Role:** GCON overwrite batch; Part-1 requires `deploy.end == update.start`.

### Aggregate ST leaves (AggStateTransition family)

-   **Files:** `state_layout_append.rs` (`StateLayoutAppendCircuit`), `state_layout_append_aggregate.rs`, `state_layout_append_wrapper.rs` / `CanonicalStateLayoutAppendWrapperCircuit`
-   **Role:** Binary aggregation of registration / deploy / update state-transition proofs (types 1 / 19 / 23 and dummies 48 / 54 / 61 in cache). PI layout matches trackable agg leaf (commitment + worker + stats + whitelist + ST hash — see ProvingJobs §9).

### `VerifyAggUserRegistartionDeployContractsGUTACircuit` (type 40, Part-1)
...
-   **File:** `…/circuits/checkpoint_state_transition.rs` (core ~387)
-   **Gadgets:** CST core + child-proof verify + recursive previous CST (`VerifyRecursiveCheckpointStateTransitionProofGadget`)
-   **Role:** Final checkpoint append / chain commitment for the block.

### `QEDCheckpointStateTransitionGenesisCircuit` (type 37)

-   **File:** `…/circuits/checkpoint_state_transition_genesis.rs` (core ~121)
-   **Role:** Genesis predecessor VK/fingerprint used when `checkpoint_id == 1` in the recursive verify gadget.
