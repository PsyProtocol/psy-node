...
This protocol reference covers bridge circuits and gadgets for deposit append, checkpoint aggregation, and withdrawal claim, alongside the [circuit index](Circuits.md) and [gadget index](Gadgets.md).
...
- [8. `BridgeAggDeltaMerkleChainGadget`](#8-bridgeaggdeltamerklechaingadget)
- [9. Flow](#9-flow)
...
## 8. `BridgeAggDeltaMerkleChainGadget`

-   **File:** `psy_plonky2_circuits/src/bridge/gadgets/bridge_agg_delta_merkle_chain.rs` (core ~63)
-   **Purpose:** Fixed **10**-slot append-only delta chain over the checkpoint tree + final leaf preimage hash binding.
-   **Constraints (pseudocode):**

    ```text
    for i in 1..10:
      delta[i].old_root == delta[i-1].new_root
      delta[i].index == delta[i-1].index + 1
    Poseidon(final_leaf_preimage) == delta[9].new_value
    ```

-   **Currency:** **No production caller** in this worktree. Live `BridgeAggChainCircuit` / `BridgeAggFinalCircuit` inline **32**-slot (`BRIDGE_AGG_CHAIN_MAX_SLOTS`) rolling logic instead. Keep for inventory; do not assume this gadget is on the L1 path.
...
Leaf hash asymmetry: deposit leaf preimage on L1 uses keccak for the Solidity leaf; Plonky2 deposit batch uses Poseidon over the deposit word layout for the frontier tree. Withdrawal leaf preimage is Poseidon end-to-end. See [`docs/src/dev/deposit-withdrawal.md`](../dev/deposit-withdrawal.md) and `encoding` notes in memory architecture docs.
