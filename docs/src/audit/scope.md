...
**Local circuit docs / §1.7:** constraint pseudocode, PI/witness, and core LOC are measured from the current worktree (see `docs/src/protocol/*Circuits*.md` and `*Gadgets.md`). Relative links in §1.7 point at local paths.
...
### 1.7 Circuit constraint briefs (local worktree)

Source of truth: this worktree. **Core lines** = non-blank, non-`//`, non-block-comment, non-`#[cfg(test)]` LOC. Links cite core gadget/circuit ranges. Narrative docs: `docs/src/protocol/{UPSGadgets,GUTAV2Circuits,CoordinatorGadgets,BridgeCircuits,PrivacyCircuits,CommonMerkleGadgets}.md`.

#### Counting rule

```text
core_loc(path) =
  lines with code after stripping // and /* */ comments,
  excluding blank lines and #[cfg(test)] modules
```

#### A. UPS gadgets / circuits

| Unit | Core LOC | Path range |
|---|---:|---|
| `CorrectUPSHeaderHashesGadget` | 17 | [`correct_header_hashes.rs:1-20`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/correct_header_hashes.rs) |
| `UPSVerifyCFCProofExistsAndValidGadget` | 101 | [`ups_cfc_verify_inclusion.rs:1-142`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_cfc_verify_inclusion.rs) |
| `UPSCFCStandardStateDeltaGadget` | 295 | [`ups_standard_cfc_state_delta.rs:1-425`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_standard_cfc_state_delta.rs) |
| `UPSVerifyCFCStandardStepGadget` | 114 | [`ups_cfc_standard.rs:1-145`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_cfc_standard.rs) |
| `UPSVerifyPopDeferredTxStepGadget` | 112 | [`ups_cfc_standard_pop_deferred_tx.rs:1-139`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_cfc_standard_pop_deferred_tx.rs) |
| `PsyUserProvingSessionSignatureDataCompactGadget` | 120 | [`ups_signature_data.rs:1-132`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_signature_data.rs) |
| `UPSEndCapResultCompactGadget` | 87 | [`ups_end_cap_result.rs:1-95`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_end_cap_result.rs) |
| `UPSEndCapCoreGadget` | 226 | [`ups_end_cap.rs:1-263`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_end_cap.rs) |
| `VerifyPreviousUPSStepProofInProofTreeGadget` | 73 | [`verify_previous_ups_step.rs:1-95`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/verify_previous_ups_step.rs) |
| Partial previous-step gadget | 85 | [`verify_previous_ups_step_partial_from_current.rs:1-107`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/verify_previous_ups_step_partial_from_current.rs) |
| `UPSEndCapFromProofTreeGadget` | 139 | [`ups_end_cap_tree.rs:1-160`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_end_cap_tree.rs) |
| `UPSStartStepGadget` | 139 | [`ups_start.rs:1-224`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_start.rs) |
| `UPSStartStepRegisterUserGadget` | 175 | [`ups_start_register_user.rs:1-284`](client_prover/psy_circuit/psy_network_circuit/src/ups/gadgets/ups_start_register_user.rs) |
| `UPSStartSessionCircuit` | 108 | [`ups/circuits/ups_start.rs:1-128`](client_prover/psy_circuit/psy_network_circuit/src/ups/circuits/ups_start.rs) |
| `UPSStartSessionRegisterUserCircuit` | 113 | [`ups/circuits/ups_start_register_user.rs:1-133`](client_prover/psy_circuit/psy_network_circuit/src/ups/circuits/ups_start_register_user.rs) |
| `UPSCFCStandardTransactionCircuit` | 115 | [`ups_cfc_standard.rs:1-139`](client_prover/psy_circuit/psy_network_circuit/src/ups/circuits/ups_cfc_standard.rs) |
| `UPSCFCDeferredTransactionCircuit` | 123 | [`ups_cfc_deferred_tx.rs:1-144`](client_prover/psy_circuit/psy_network_circuit/src/ups/circuits/ups_cfc_deferred_tx.rs) |
| `UPSStandardEndCapCircuit` | 238 | [`end_cap.rs:1-270`](client_prover/psy_circuit/psy_network_circuit/src/ups/circuits/end_cap.rs) |

**`UPSStartStepGadget` — role / IO / constraints**

* Role: Bootstrap UPS from a finalized checkpoint + user leaf.
* Public inputs (circuit): tree-aware hash of starting header over empty proof-tree root.
* Private / witness: UPS header; checkpoint leaf; global state roots; checkpoint-tree Merkle proof; user-tree Merkle proof.
* Pseudocode:
```text
checkpoint_proof.root  == header.checkpoint_tree_root
checkpoint_proof.value == header.checkpoint_leaf_hash
checkpoint_proof.index == header.checkpoint_id
hash(checkpoint_leaf)  == header.checkpoint_leaf_hash
user_proof.root == state_roots.user_tree_root
user_proof.value == header.start_session_user_leaf_hash
current.user_leaf.last_checkpoint_id := checkpoint_id
deferred_root == empty; inline_root == empty; tx_count == 0
```

**`UPSStartStepRegisterUserGadget` — role / IO / constraints**

* Role: Bootstrap UPS for a **new** user (registration-tree membership + empty GUSR leaf).
* Public inputs (circuit): tree-aware hash of starting header over empty proof-tree root (separate whitelist fingerprint from registered-user start).
* Private / witness: as start, plus `user_registration_tree_proof` / index bits.
* Pseudocode:
```text
registration_proof.root == state_roots.user_registration_tree_root
registration_proof.value != ZERO
user_id == bits_to_user_id(registration index)
start_user_leaf: DEFAULT_USER_STATE_TREE_ROOT; balance/nonce/… == 0
public_key == registration_proof.value
user_tree_proof.value == ZERO; index == user_id
// current_state empty debt/tx init as start
```

**`UPSCFCStandardStateDeltaGadget` — role / IO / constraints**

* Role: Apply one CFC’s claimed start/end context to UCON / debt pivots / tx stack.
* Public inputs: none (new header becomes next PI preimage).
* Private / witness: CFC tx context; UCON `DeltaMerkleProof`; deferred/inline historical pivots.
* Pseudocode:
```text
cfc_inner_PI_hash := hash(tx_input_context)
UCON.old_root == previous.user_state_tree_root
UCON.index == tx.contract_id
UCON.new_value == tx.end_contract_state_root
deferred historical_root == corrections.deferred; current_root == end_deferred
tx_count' = tx_count + 1; push call_data onto tx_hash_stack
new_header = Header(whitelist, start_ctx, updated current_state)
```

**`UPSStandardEndCapCircuit` — role / IO / constraints**

* Role: Close session; bind ZK signature; empty debts; fee burn; emit EndCap result + GUTA stats.
* Public inputs: `H(state_transition_pi_hash, guta_stats_pi_hash)`.
* Private / witness: previous UPS step attest; ZK-sig leaf; pubkey param; nonce; slots_modified; agg whitelist/root proof.
* Pseudocode:
```text
verify previous UPS step + whitelist
verify ZK-sig in same proof tree
nonce' > start.nonce; pk stable; debts empty
last tx == TOKEN burn(GUTA_FEE + DA_FEE * slots)
agg.state_transition_end == current_proof_tree_root
```

#### B. GUTA / RealmFinalize

| Unit | Core LOC | Path range |
|---|---:|---|
| `VerifyEndCapProofGadget` | 127 | [`verify_end_cap.rs:22-162`](psy_plonky2_circuits/src/guta/gadgets/verify_end_cap.rs) |
| `VerifyGUTAProofGadget` | 123 | [`verify_guta_proof.rs:20-153`](psy_plonky2_circuits/src/guta/gadgets/verify_guta_proof.rs) |
| `DualVariableHeightStateTransitionGadget` | 114 | [`dual_variable_height_state_transition.rs:12-155`](psy_plonky2_circuits/src/guta/gadgets/dual_variable_height_state_transition.rs) |
| DVH Merkle engine | 208 | [`dual_variable_height_delta_merkle_proof.rs:16-200`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/dual_variable_height_delta_merkle_proof.rs) |
| TwoGUTA / TwoEndCap / SingleEndCap / LeftGUTA+RightEndCap / Linear | ~188–209 each | [`guta_v2/circuits/*.rs`](psy_plonky2_circuits/src/guta_v2/circuits) (live cache; types 9/12/14/64 absent) |
| `GUTANoChangeCircuit` (15) | ~184 | [`guta_no_change.rs`](psy_plonky2_circuits/src/guta/circuits/guta_no_change.rs) |
| `GUTAVerifyGUTAToCapCircuit` (13) | ~159 | [`verify_guta_to_cap.rs`](psy_plonky2_circuits/src/guta/circuits/verify_guta_to_cap.rs) |
| `GUTAVerifyGUTAToCapUpgradeCheckpointCircuit` (56) | ~183 | [`verify_guta_to_cap_upgrade_checkpoint.rs`](psy_plonky2_circuits/src/guta/circuits/verify_guta_to_cap_upgrade_checkpoint.rs) |
| `RealmFinalizeGUTACircuit` | ~896 | [`realm_finalize_guta.rs:203-754`](psy_plonky2_circuits/src/guta_v2/circuits/realm_finalize_guta.rs) |

**Shared GUTA PI:** 4 felts = `H2(header_hash, rewards_tree_value)`.

**`VerifyGUTAProofGadget`**
```text
verify_proof(child)
header.whitelist_root == whitelist_merkle.root
expected_PI = H2(header_hash, rewards) == proof.PI[0..4]
whitelist_merkle.value == fingerprint(vk)
```

**`DualVariableHeightStateTransitionGadget`**
```text
DVH(A.old/new/index, B.old/new/index); left.new_root == right.old_root
A.checkpoint == B.checkpoint; A.whitelist == B.whitelist; A.level == B.level
new_level = level - DVH.height
new_header = {old=DVH.old, new=DVH.new, combine(stats), agg+1}
```

**`RealmFinalizeGUTACircuit`**
```text
root = VerifyGUTA; level == coordinator_global_user_tree_height
enforce_rotation(...); prove checkpoint + old realm root + validator leaf
# no child WrappedSignatureProof (64) — BLS auth cutover; PK non-zero check remains
new_balance = validator.balance + da_fees; delta-update realm subtree
PI = H2(final_header_hash, H2(H2(root.rewards, output_commitment), worker_tag))
```
Off-circuit auth: realm BLS Proposal/Certificate. See `docs/src/protocol/ProvingJobs.md` §2.
#### C. Coordinator Part-1 / CST

| Unit | Core LOC | Path range |
|---|---:|---|
| `BatchAppendUserRegistrationTreeGadget` | 80 | [`append_user_registration_tree.rs:18-94`](psy_plonky2_circuits/src/coordinator/gadgets/append_user_registration_tree.rs) |
| `BatchDeployContractsGadget` (v1) | 57 | [`deploy_contract.rs:17-71`](psy_plonky2_circuits/src/coordinator/gadgets/deploy_contract.rs) |
| `BatchDeployContractsGadget` (v2 layout) | ~402 | [`deploy_contract_v2.rs`](psy_plonky2_circuits/src/coordinator/gadgets/deploy_contract_v2.rs) |
| `BatchUpdateContractsGadget` | ~263 | [`update_contract.rs`](psy_plonky2_circuits/src/coordinator/gadgets/update_contract.rs) |
| `VerifyAggUserRegistartionDeployContractsGUTAGadget` | ~309 | [`verify_agg_user_registration_deploy_guta.rs`](psy_plonky2_circuits/src/coordinator/gadgets/verify_agg_user_registration_deploy_guta.rs) |
| `QEDPart1StateDeltaResultGadget` | ~221 | [`checkpoint_state_transition_proofs.rs`](psy_plonky2_circuits/src/coordinator/gadgets/checkpoint_state_transition_proofs.rs) |
| `CheckpointStateTransitionCoreGadget` | ~234 | [`checkpoint_state_transition.rs:197-255`](psy_plonky2_circuits/src/coordinator/gadgets/checkpoint_state_transition.rs) |
| `VerifyRecursiveCheckpointStateTransitionProofGadget` | ~79 | [`recursive_checkpoint_state_transition_verify.rs`](psy_plonky2_circuits/src/coordinator/gadgets/recursive_checkpoint_state_transition_verify.rs) |
| `BatchAppendUserRegistrationTreeCircuit` (0) | ~191 | [`batch_append_user_registration_tree.rs`](psy_plonky2_circuits/src/coordinator/circuits/batch_append_user_registration_tree.rs) |
| `BatchDeployContractsCircuit` (18) | ~192 / v2 wrapper | [`batch_deploy_contract.rs`](psy_plonky2_circuits/src/coordinator/circuits/batch_deploy_contract.rs), [`batch_deploy_contract_v2.rs`](psy_plonky2_circuits/src/coordinator/circuits/batch_deploy_contract_v2.rs) |
| `BatchUpdateContractsCircuit` (22) | ~219 | [`batch_update_contract.rs`](psy_plonky2_circuits/src/coordinator/circuits/batch_update_contract.rs) |
| Agg ST / layout-append family | — | [`state_layout_append*.rs`](psy_plonky2_circuits/src/coordinator/circuits) |
| CST circuit (32) | ~387 | [`checkpoint_state_transition.rs`](psy_plonky2_circuits/src/coordinator/circuits/checkpoint_state_transition.rs) |
| Genesis CST (37) | ~121 | [`checkpoint_state_transition_genesis.rs`](psy_plonky2_circuits/src/coordinator/circuits/checkpoint_state_transition_genesis.rs) |
| Part-1 Agg circuit (40) | ~382 | [`agg_user_registration_deploy_guta.rs`](psy_plonky2_circuits/src/coordinator/circuits/agg_user_registration_deploy_guta.rs) |

**Part-1 verify gadget**
```text
verify RegisterUsers ST; Deploy ST; Update ST; VerifyGUTA
whitelist root constant
deploy.end_root == update.start_root
header.gcon = (deploy.start, update.end)
```

**CST core**
```text
append.old_value == ZERO
append.old_root == previous_checkpoint_proof.root
append.index == previous.index + 1
```
...
| Unit | Core LOC | Path range |
|---|---:|---|
| `DepositBatchAppendCircuit` | ~490 | [`deposit_batch_append_circuit.rs:243-390`](psy_plonky2_common_circuits/src/bridge/deposit_batch_append_circuit.rs) |
| `WithdrawalBatchClaimCircuit` | ~285 | [`withdrawal_batch_claim_circuit.rs:60-184`](psy_plonky2_common_circuits/src/bridge/withdrawal_batch_claim_circuit.rs) |
| `BridgeAggChainCircuit` | ~481 | [`bridge_agg_chain.rs:78-298`](psy_plonky2_circuits/src/bridge/circuits/bridge_agg_chain.rs) |
| `BridgeAggFinalCircuit` | ~539 | [`bridge_agg_final.rs:74-329`](psy_plonky2_circuits/src/bridge/circuits/bridge_agg_final.rs) |
| Wrap circuits | ~191 | [`bridge_wrap.rs`](psy_plonky2_circuits/src/bridge/circuits/bridge_wrap.rs) |
| `TreeRootInContractStateGadget` | ~117 | [`tree_root_in_contract_state.rs:53-132`](psy_plonky2_circuits/src/bridge/gadgets/tree_root_in_contract_state.rs) |
| `BridgeAggDeltaMerkleChainGadget` | ~63 | [`bridge_agg_delta_merkle_chain.rs`](psy_plonky2_circuits/src/bridge/gadgets/bridge_agg_delta_merkle_chain.rs) (**no caller**; live path uses 32-slot inline) |

**Deposit batch**
```text
for i in 0..32: leaf = Poseidon(deposit_words); FrontierAppend; mask inactive
batch_commit = keccak(all slot words)
PI = keccak(old/new root || from/to || leaves || frontiers || bridge_uid || batch_commit)
```

**BridgeAgg Final**
```text
verify Chain; fingerprint known
append terminal deltas; verify checkpoint ST proof
rolling_chain_hash == checkpoint_proof.PI_hash
extract deposit/withdrawal roots via TreeRootInContractState
register 26 flat PIs for L1
```
...
| Unit | Core LOC | Path range |
|---|---:|---|
| `DepositInclusionCircuit` / `ShieldDepositClaimCircuit` | ~384 | [`deposit_inclusion.rs:31-181`](client_prover/psy_circuit/psy_dpn_circuit/src/circuits/privacy/deposit_inclusion.rs) |
| `PrivateNoteInclusionCircuit` | ~423 | [`private_note_inclusion.rs:47-181`](client_prover/psy_circuit/psy_dpn_circuit/src/circuits/privacy/private_note_inclusion.rs) |
| CFC `DapenContractFunctionCircuit` | ~137 | [`cfc.rs`](client_prover/psy_circuit/psy_dpn_circuit/src/circuits/cfc.rs) |
| `PsyContractFunctionBuilderGadget` | ~227 | [`vm/compile.rs`](client_prover/psy_circuit/psy_dpn_circuit/src/vm/compile.rs) |
| `StateReaderGadget` (DPN VM) | ~1930 | [`vm/gadgets/state_readers.rs`](client_prover/psy_circuit/psy_dpn_circuit/src/vm/gadgets/state_readers.rs) — see [PrivacyCircuits.md](docs/src/protocol/PrivacyCircuits.md) §5 |

**Deposit inclusion**
```text
nullifier = Poseidon(nullifier_secret)
note_commitment = Poseidon(nullifier_secret || note_secret)
deposit_commitment = Poseidon(shield || token || l2 || amount || chain || note)
Merkle(h=32) → deposit_root
PI = Poseidon(shield || amount || token || l2 || chain || root || nullifier || note || index)
```

#### F. Common Merkle (node stack)

| Unit | Core LOC | Path |
|---|---:|---|
| `MerkleProofGadget` | ~289 | [`merkle_proof.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/merkle_proof.rs) |
| `DeltaMerkleProofGadget` | ~524 | [`delta_merkle_proof.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/delta_merkle_proof.rs) |
| `HistoricalRootMerkleProofGadget` | ~163 | [`historical_root_merkle_proof.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/historical_root_merkle_proof.rs) |
| `SpidermanAppendProofGadget` | ~91 | [`spiderman_append_proof.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/spiderman_append_proof.rs) |
| `QVariableHeightDeltaMerkleProofGadget` | ~169 | [`q_variable_height_delta_merkle_proof.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/q_variable_height_delta_merkle_proof.rs) |
| `FrontierAppendGadget` | ~165 | [`frontier_append.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/frontier_append.rs) |
| `FullMerkleTreeAppendGadget` | ~229 | [`full_merkle_tree_append.rs`](psy_plonky2_common_circuits/src/hash/merkle/gadgets/full_merkle_tree_append.rs) |
| Client `VariableHeightMerkleProofGadget` | ~219 | [`variable_height_merkle_proof.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/variable_height_merkle_proof.rs) |
| Client `VariableHeightDeltaMerkleProofGadget` | ~183 | [`variable_height_delta_merkle_proof.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/variable_height_delta_merkle_proof.rs) |
| Client `VariableHeightDeltaMerkleProofOptGadget` | ~386 | [`variable_height_delta_merkle_proof_opt.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/variable_height_delta_merkle_proof_opt.rs) |
| Client `VariableHeightDeltaMerkleProofOptV2Gadget` | ~212 | [`variable_height_delta_merkle_proof_opt_v2.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/variable_height_delta_merkle_proof_opt_v2.rs) |
| Client `SubSlotDeltaMerkleProofBatchGadget` | ~260 | [`sub_slot_delta_merkle_proof_batch.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/sub_slot_delta_merkle_proof_batch.rs) |
| Client `IMTUpdate` / `IMTInsert` | ~228 | [`imt_contract_state_update.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/imt_contract_state_update.rs) |
| Client NCA (`UpdateNearestCommonAncestor*`) | ~124 / opt | [`sub_tree_update_proof.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/sub_tree_update_proof.rs), [`sub_tree_update_proof_opt.rs`](client_prover/psy_circuit/psy_common_circuit/src/hash/merkle/gadgets/sub_tree_update_proof_opt.rs) |

Client VH/IMT/NCA narrative: [CommonMerkleGadgets.md](docs/src/protocol/CommonMerkleGadgets.md) §9–§12. Node vs client stacks must not be mixed in one proof.
...
Inventory tables above historically used `origin/mainnet-beta`. Section 1.7 and protocol docs use **this local worktree** as SoT for circuit constraint briefs and core LOC (intended for a later push). Reconcile pin SHAs before external handoff.
