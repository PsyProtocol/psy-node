...
use crate::data::{fixed_serializable::QPDFixedSizeSerializable, hash::hash256::Hash256, serializable::QPDSerializable};
...
pub struct TempTablePrefixIdentifierRealm<const TABLE_TYPE: u16, const KEY_SIZE: usize, const COMBINED_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE>> {
    pub realm_id: u32,
...
impl<const TABLE_TYPE: u16, const KEY_SIZE: usize, const COMBINED_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE>> TempTablePrefixIdentifierRealm<TABLE_TYPE, KEY_SIZE, COMBINED_SIZE, K> {
    pub fn new(realm_id: u32, realm_sub_id: u16) -> Self {
        ...
        Self { realm_id, realm_sub_id, prefix_template_u64, prefix_template_bytes, _k_marker: std::marker::PhantomData }
    }
}
impl<const TABLE_TYPE: u16, const KEY_SIZE: usize, const COMBINED_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE>> TempTablePrefixIdentifierCoreBaseForKey<COMBINED_SIZE, K> for TempTablePrefixIdentifierRealm<TABLE_TYPE, KEY_SIZE, COMBINED_SIZE, K> {
    fn ttp_get_tablet_prefix_size() -> usize {
        8
    }
    fn ttp_get_inner_key_size() -> usize {
    ...
    fn ttp_from_full_key_bytes_with_prefix(key_bytes: &[u8]) -> anyhow::Result<(Self, K)> {
        ...
        Ok((Self { realm_id, realm_sub_id, prefix_template_u64, prefix_template_bytes, _k_marker: std::marker::PhantomData }, key))
    }
    
}
impl<const TABLE_TYPE: u16, const KEY_SIZE: usize, const COMBINED_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE>> TempTablePrefixAutoIdentifierCoreBaseForKey<COMBINED_SIZE, K> for TempTablePrefixIdentifierRealm<TABLE_TYPE, KEY_SIZE, COMBINED_SIZE, K> {
}
...
#[derive(Debug, Clone)]
pub struct QSTempTableDefintionRealm<const TABLE_TYPE: u16, const COMBINED_KEY_SIZE: usize, const KEY_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE> + Send + Sync, V: TTPSerializeValue + Clone + Send + Sync> {
    key_prefix: TempTablePrefixIdentifierRealm<TABLE_TYPE, KEY_SIZE, COMBINED_KEY_SIZE, K>,
    _v_marker: std::marker::PhantomData<V>,
}

impl<const TABLE_TYPE: u16, const COMBINED_KEY_SIZE: usize, const KEY_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE> + Send + Sync, V: TTPSerializeValue + Clone + Send + Sync> QSTempTableDefintionRealm<TABLE_TYPE, COMBINED_KEY_SIZE, KEY_SIZE, K, V> {
    pub fn new(realm_id: u32, realm_sub_id: u16) -> Self {
        Self {
...
impl<const TABLE_TYPE: u16, const COMBINED_KEY_SIZE: usize, const KEY_SIZE: usize, K: TempTableInnerKeyBase<KEY_SIZE> + Copy + Send + Sync, V: TTPSerializeValue + Clone + Send + Sync> TempTableDefintion<COMBINED_KEY_SIZE, KEY_SIZE> for QSTempTableDefintionRealm<TABLE_TYPE, COMBINED_KEY_SIZE, KEY_SIZE, K, V> {
    type Key = K;
    type Value = V;
