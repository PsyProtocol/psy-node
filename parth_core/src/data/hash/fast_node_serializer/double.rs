...
impl QMerkleStoreFastDoubleNodeSerializer {
    ...
    pub fn serialize_double_id_hash_map_with_common_tree_id_to_vec<Hash: Q256BitHash>(tree_id: u64, tree_sub_id: u64, node_map: &HashMap<SimpleMerkleNodeKey, Hash>) -> Vec<u8> {
        let mut serialized_bytes = Vec::with_capacity(node_map.len() * QMS_FAST_SERIALIZER_DOUBLE_ID_NODE_SIZE);
        let tree_id_bytes = tree_id.to_le_bytes();
        let tree_sub_id_bytes = tree_sub_id.to_le_bytes();
        for (key, hash) in node_map {
            serialized_bytes.extend_from_slice(&tree_id_bytes);
            serialized_bytes.extend_from_slice(&tree_sub_id_bytes);
    ...
    pub fn serialize_double_id_hash_map_with_common_tree_id_to_slice<Hash: Q256BitHash>(tree_id: u64, tree_sub_id: u64, node_map: &HashMap<SimpleMerkleNodeKey, Hash>, serialized_bytes: &mut Vec<u8>) {
        let tree_id_bytes = tree_id.to_le_bytes();
        let tree_sub_id_bytes = tree_sub_id.to_le_bytes();
        for (key, hash) in node_map {
            serialized_bytes.extend_from_slice(&tree_id_bytes);
