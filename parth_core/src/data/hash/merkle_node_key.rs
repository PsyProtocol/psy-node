...
fn build_recursive_v1(
    ...
    // --- Conquer Phase ---
    let left_result = build_recursive_v1(left_nodes, subtree_root.left_child(), tree_height, aggregations);
    let right_result = build_recursive_v1(right_nodes, right_child, tree_height, aggregations);
...
pub fn generate_nca_tree_groups_v1(leaves: &[SimpleMerkleNodeKey], _leaf_level: u8) -> Vec<Vec<SimpleMerkleNodeNCAAggregation>> {
    ...
    // aggregation.
    build_recursive_v1(&sorted_leaves, root_node, tree_height, &mut aggregations);
...
fn build_recursive_v0(
    ...
    let left_nca = build_recursive_v0(left_nodes, subtree_root.left_child(), tree_height, aggregations);
    let right_nca = build_recursive_v0(right_nodes, right_child, tree_height, aggregations);
...
pub fn generate_nca_tree_v0(leaves: &[SimpleMerkleNodeKey]) -> Vec<SimpleMerkleNodeNCAAggregation> {
    if leaves.len() < 2 {
        return vec![];
    }
    ...
    build_recursive_v0(&sorted_leaves, root_node, tree_height, &mut aggregations);

    aggregations
}

pub fn generate_nca_tree_groups_v0(leaves: &[SimpleMerkleNodeKey], leaf_level: u8) -> Vec<Vec<SimpleMerkleNodeNCAAggregation>> {
    let mut ncas = generate_nca_tree_v0(leaves);
    let mut nca_map: std::collections::HashMap<SimpleMerkleNodeKey, SimpleMerkleNodeNCAAggregation> = std::collections::HashMap::new();
...
mod tests_old {
    ...
    use crate::data::hash::merkle_node_key::{
        check_nca_tree_groups, generate_nca_tree_v0, generate_nca_tree_groups_v1, generate_nca_tree_groups_v0, generate_nca_tree_groups_v2, SimpleMerkleNodeKey, SimpleMerkleNodeNCAAggregation
    };
    ...
    fn random_nodes_test_gen(count: usize, height: u8) {
        ...
        let ncas = generate_nca_tree_v0(&leaves);
    ...
    fn check_tree_groups() {
        ...
        let groups = generate_nca_tree_groups_v0(&leaves, height);
    ...
    fn check_tree_groups_2() {
        ...
        let n_group_levels = generate_nca_tree_groups_v0(&leaves, guta_height);
    ...
    fn run_many_random_nodes() {
        ...
        let ncas = generate_nca_tree_v0(&nodes);
    ...
    fn test_with_prompt_example() {
        ...
        let path = generate_nca_tree_v0(&leaves);
