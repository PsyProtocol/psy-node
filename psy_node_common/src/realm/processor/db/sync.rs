    ...
    N::HasherBase: 'static + Send + Sync,
{
    pub async fn sync_to_coordinator_set_checkpoint_id(&mut self) -> anyhow::Result<()> {
        ...
        tracing::info!(
            "Syncing checkpoint metadata from checkpoint ID {} to {}.",
        ...
        let latest_sync_info: PsyRealmCoordinatorUpdate<N::F, N::QHash> =
            self.coordinator_client.rc_get_realm_sync_info(latest_checkpoint_id).await?;
        self.db
        ...
        if latest_db_l2_info.next_contract_id != latest_sync_info.checkpoint_sync_info.block_state.next_contract_id {
            ...
            let full_batches =
                (latest_sync_info.checkpoint_sync_info.block_state.next_contract_id - latest_db_l2_info.next_contract_id) / (batch_size as u32);
            let remainder =
                (latest_sync_info.checkpoint_sync_info.block_state.next_contract_id - latest_db_l2_info.next_contract_id) % (batch_size as u32);
            for i in 0..(full_batches as u64) {
                ...
                let heights: Vec<(u64, u8)> = self
                    .coordinator_client
                    .rc_get_contract_tree_state_heights(latest_checkpoint_id, (start_id..end_id).collect())
                    .await?
                    .into_iter()
                    .enumerate()
                    .map(|(i, b)| (i as u64 + start_id, b))
                    .collect();
                println!(
                    "Fetched contract tree heights for contract IDs {} to {} during sync to coordinator {:?}.",
                    start_id, end_id, heights
                );

                self.db.set_contract_tree_heights(latest_checkpoint_id, &heights).await?;
                tracing::info!(
                    "Initialized contract state tree leaves for contract IDs {} to {} during sync to coordinator.",
                    start_id,
                    end_id
                );
            }
    ...
    pub async fn wait_for_realm_update_sync_with_coordinator(
        ...
        loop {
            ...
            if latest_realm_root.value == new_realm_root {
                tracing::info!(
                    "Realm root has been updated to the new value: {:?} at checkpoint ID: {}",
                    new_realm_root,
                    latest_realm_root.checkpoint_id
                );
                ...
                let sync_info: PsyRealmCoordinatorUpdate<N::F, N::QHash> =
                    self.coordinator_client.rc_get_realm_sync_info(latest_realm_root.checkpoint_id).await?;
                self.db
                    .set_realm_rewards_tag_tree_top_proof_at_checkpoint_id(latest_realm_root.checkpoint_id, &sync_info.reward_tree_top_proof)
                    .await?;
                self.db
                    .global_user_tree_set_top_tree_merkle_proof(latest_realm_root.checkpoint_id, &sync_info.merkle_proof_to_realm_root)
                    .await?;
                self.db
                    ...
                    .await?;
                self.db
                    .set_l2_block_state(latest_realm_root.checkpoint_id, &sync_info.checkpoint_sync_info.block_state)
                    .await?;
                self.db.set_l2_latest_block_state(&sync_info.checkpoint_sync_info.block_state).await?;
                self.db
                    .set_checkpoint_global_state_roots(latest_realm_root.checkpoint_id, &sync_info.checkpoint_sync_info.state_roots)
                    .await?;
                self.db
                    .set_checkpoint_leaf_data(latest_realm_root.checkpoint_id, &sync_info.checkpoint_sync_info.checkpoint_leaf)
                    .await?;
                return Ok(sync_info);
            } else {
                tracing::info!(
                    "Waiting for realm root to be updated to the new value: {:?}. Current realm root at checkpoint ID {} is {:?}. Retrying...",
                    new_realm_root,
                    latest_realm_root.checkpoint_id,
                    latest_realm_root.value
                );
