    ...
    N: 'static, FileSystem::File: Send + Sync + 'static,
{
    ...
    // The same call `init` already made, kept because a Realm that reaches the
    // runner without going through it must not start on an unreconciled cache.
    // It restarts if it adopts an epoch; by here it usually has nothing to do.
    if let Err(e) = processor.db.reconcile_missed_rollback_epochs_or_restart().await {
        tracing::error!(
