...
impl<F: QFelt64, Hash: Q256BitHash + QFHashBase<F>> CoordinatorGUTAPlanner<F, Hash> {
    ...
    /// level vector (dep_index) relative to the parent's reward index.
       fn update_reward_tree_config(
        ...
        // If not found in job_levels, it's likely an input leaf (from input_buffer)
        // or a job that wasn't generated by this planner run.
        // Input leaves in the buffer don't need their metadata updated in this vector
        // because they aren't part of the output result set.
        if !found_in_levels {
            return Ok(());
        }

        // 2. Iterate through dependencies based on their position in the parent's list.
        // The child's index is calculated as: parent_index * 2 + child_position_in_parent_deps
        for (child_pos, dep_job_id) in deps.into_iter().enumerate() {
            let child_reward_index = (index << 1) + (child_pos as u64);
            
            // Recursively update the dependency
