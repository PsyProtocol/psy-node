    ...
    CheckpointTreeBackupManager<Hasher, Hash, FileSystem>
{
    ...
    pub async fn append_checkpoint_leaf_hash(&mut self, checkpoint_id: u64, checkpoint_hash: Hash) -> anyhow::Result<DeltaMerkleProofCore<Hash>> {
        ...
        self.file_system
            ...
            .await?;
        tracing::info!(
            "Appended checkpoint id {} to checkpoint tree backup file at path {}",
            checkpoint_id,
            self.backup_file_path
        );
...
mod test {
    ...
    #[async_trait::async_trait]
    impl PsyNodeCheckpointTreeDatabaseWriter<Hash> for SimpleMockDBProvider {
        async fn checkpoint_tree_set_leaf_hash(&self, checkpoint_id: u64, value: Hash) -> anyhow::Result<DeltaMerkleProofCore<Hash>> {
            Ok(self.tree.set_leaf(checkpoint_id, value))
        }
        async fn checkpoint_tree_set_nodes(&self, _checkpoint_id: u64, nodes: &[SimpleMerkleNode<Hash>]) -> anyhow::Result<()> {
            for node in nodes {
                self.tree.set_node_value(node.key, node.value);
            }
            Ok(())
        }
    }

    pub struct CheckpointTreeTestHelper {
        pub mock_db: Arc<SimpleMockDBProvider>,
        pub fs: Arc<SimpleMockMemoryFileSystem>,
        pub file_path: String,
    }

    impl CheckpointTreeTestHelper {
        pub fn new(file_path: &str) -> Self {
            let mock_db = Arc::new(SimpleMockDBProvider {
                tree: PsyDashMemoryMerkleStore::<Hasher, Hash>::new(32),
            });
        ...
        pub fn get_file_binary(&self) -> anyhow::Result<Vec<u8>> {
            match self.fs.files.get(&self.file_path) {
                Some(file) => Ok(file.value().clone()),
                None => anyhow::bail!("File not found in mock file system"),
            }
        }
        pub fn create_with_db_leaves(start_checkpoint_id: u64, leaves: &[Hash]) -> anyhow::Result<Self> {
            let helper = CheckpointTreeTestHelper::new("checkpoint_backup_test_file.chkpt");
            for (i, leaf) in leaves.iter().enumerate() {
                helper.mock_db.tree.set_leaf(start_checkpoint_id + i as u64, *leaf);
            }
            Ok(helper)
        }
    }
