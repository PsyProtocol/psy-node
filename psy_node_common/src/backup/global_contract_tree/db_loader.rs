...
>(
    ...
    let mut current_value = contract_db_reader.global_contract_tree_get_leaf_hash(checkpoint_id, current_key.index).await?;
    ...
    while current_key.level < tree_height {
        ...
        let right_child_value = contract_db_reader.global_contract_tree_get_leaf_hash(checkpoint_id, right_child_key.index).await?;
        ...
        if !right_is_empty {
            ...
            let left_child_value = contract_db_reader.global_contract_tree_get_leaf_hash(checkpoint_id, left_child_key.index).await?;
    ...
    let merkle_proof_a = contract_db_reader.global_contract_tree_get_merkle_proof(checkpoint_id, current_key.index).await?;
    let next_contract_id = current_key.index + 1;
    let merkle_proof_b = contract_db_reader.global_contract_tree_get_merkle_proof(checkpoint_id, next_contract_id).await?;
