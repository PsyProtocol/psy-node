...
/// Validates that `public_key` is a 33-byte compressed secp256k1 public key with
/// a valid parity prefix (`0x02` or `0x03`).
pub fn validate_compressed_secp256k1_public_key(public_key: CompressedPublicKey) -> anyhow::Result<CompressedPublicKey> {
    anyhow::ensure!(public_key.0.len() == 33, "compressed secp256k1 public key must be 33 bytes");
    anyhow::ensure!(
        public_key.0[0] == 0x02 || public_key.0[0] == 0x03,
        "invalid compressed secp256k1 public key prefix: expected 0x02 or 0x03"
    );
    Ok(public_key)
}

/// A recovered EIP-191 signature: the compressed secp256k1 public key.
pub struct RecoveredEthPersonalSignature {
    pub public_key: [u8; 33],
}

/// Returns the network-bound EIP-191 registration challenge for an Ethereum
/// address. The challenge is `personal_sign` digest over
/// `keccak256(network_magic || evm_address)`.
pub fn eth_personal_registration_challenge(network_magic: u64, selected_evm_address: [u8; 20]) -> Hash256 {
    use tiny_keccak::{Hasher, Keccak};

    let mut inner = Keccak::v256();
    inner.update(&network_magic.to_be_bytes());
    inner.update(&selected_evm_address);
    let mut message = [0u8; 32];
    inner.finalize(&mut message);
    Hash256(eth_personal_sign_digest(&message))
}

/// Computes the Ethereum address (20 bytes) for an uncompressed secp256k1
/// verifying key.
pub fn ethereum_address_for_verifying_key(verifying_key: &VerifyingKey) -> [u8; 20] {
    use tiny_keccak::{Hasher, Keccak};

    let uncompressed = verifying_key.to_encoded_point(false).to_bytes();
    let mut pubkey_hasher = Keccak::v256();
    pubkey_hasher.update(&uncompressed[1..]);
    let mut pubkey_hash = [0u8; 32];
    pubkey_hasher.finalize(&mut pubkey_hash);
    pubkey_hash[12..].try_into().expect("slice is 20 bytes")
}

/// Recovers the compressed secp256k1 public key from a 65-byte EIP-191
/// `personal_sign` signature and verifies that it matches the selected Ethereum
/// address. `recovery_message` must be the exact challenge bytes returned by
/// [`eth_personal_registration_challenge`].
pub fn recover_eth_personal_signature(
    selected_evm_address: [u8; 20],
    recovery_message: Hash256,
    signature: [u8; 65],
) -> anyhow::Result<RecoveredEthPersonalSignature> {
    anyhow::ensure!(
        selected_evm_address.iter().any(|byte| *byte != 0),
        "selected Ethereum address must not be the zero address"
    );

    let digest = eth_personal_sign_digest(&recovery_message.0);
    let recid = RecoveryId::try_from(signature[64])?;
    let sig = Signature::from_slice(&signature[..64])?;
    let verifying_key = VerifyingKey::recover_from_prehash(&digest, &sig, recid)?;

    let uncompressed = verifying_key.to_encoded_point(false).to_bytes();
    let compressed = uncompressed_secp256k1_to_compressed(&uncompressed)?;
    let recovered_address = ethereum_address_for_verifying_key(&verifying_key);

    anyhow::ensure!(
        recovered_address == selected_evm_address,
        "recovered Ethereum address does not match the selected address"
    );

    Ok(RecoveredEthPersonalSignature { public_key: compressed.0 })
}
