...
struct MintAgentAccountArgs {
    ...
    /// circuit, so this is a transaction shape, not a budget). Usually 1.
...
struct SpendLogArgs {
    ...
    #[serde(default)]
    policy_id: Option<String>,
    /// Owner token when the server was started with PSY_MCP_OWNER_TOKEN.
    #[serde(default)]
    owner_token: Option<String>,
}
...
impl PsyWalletServer {
            ...
            Err(e) => {
                return err_json(
                    ...
                    json!({
                        ...
                        "note": "If the failure happened after key generation the key is already backed up in keystoreDir. Minting the same capabilities again is deterministic and reproduces the same mandate circuit."
                    }),
                );
            }
        };
    ...
    #[tool(
        description = "Owner: the spends this server has AUTHORIZED, newest first — timestamp, method, recipient and \
    ...
    #[tool(description = "Owner: the BLOCKED attempts, newest first — what the agent tried to spend and the \
    ...
    async fn claim_batch(&self, Parameters(a): Parameters<ClaimBatchArgs>) -> Result<CallToolResult, McpError> {
        ...
        let deposits: Vec<(&crate::wallet::DepositNote, &serde_json::Value)> = deposit_notes
            ...
            .map(|(n, p)| (n, p))
            .collect();
        let privates: Vec<(&crate::wallet::IncomingPrivateNote, u64)> = private_parsed
            .iter()
            .map(|(n, c)| (n, *c))
            .collect();
...
async fn load_claimable_deposit(
    ...
    let bridge: alloy_primitives::Address = default_bridge()
        .parse()
        .map_err(|e| (
            format!("PSY_MCP_L1_BRIDGE is not a valid Ethereum address: {e}"),
            json!({ "gate": "config", "var": "PSY_MCP_L1_BRIDGE" }),
        ))?;
    let proved = match crate::l1::L1Client::from_env(default_l1_rpc()) {
        Ok(l1) => l1.call_u64(bridge, "provedDepositCount()").await.unwrap_or(0),
        Err(_) => 0, // read-only path also works keyless below
    };
