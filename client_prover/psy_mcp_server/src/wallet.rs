...
impl WalletManager {
    ...
    pub fn prepare_private_transfer(
        ...
        let normalized = recipient_shielded_hex
            ...
            .trim_start_matches("0X");
        let owner_hash = QHashOut::<F>::from_str(normalized)
            .map_err(|e| anyhow!("invalid recipient shielded address: {e}"))?;
...
impl WalletManager {
    ...
    pub fn receive_identity(&self) -> Result<ReceiveIdentity> {
        ...
        Ok(ReceiveIdentity {
            shield_address_hex: shield.to_string(),
            random0,
            random1,
...
impl DepositNote {
    ...
    pub fn l1_words(&self) -> Result<([u8; 32], [u8; 32])> {
        let shield = qhash_from_display_hex(&self.shield_address_hex)?;
...
impl WalletManager {
    ...
    pub async fn build_shield_deposit_claim(
        ...
        let shield = qhash_from_display_hex(&note.shield_address_hex)?;
        let token_words = {
