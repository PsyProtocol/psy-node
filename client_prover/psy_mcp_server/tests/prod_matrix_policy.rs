...
fn agent_01_a_payment_within_every_cap_is_authorized_and_logged() {
    ...
    let auth = e
        .authorize(&t, "204800", 250_000_000, "transfer")
    ...
    assert_eq!(log[0].method, "transfer");
...
fn agent_01b_a_claim_needs_no_allowlist_entry_and_costs_no_budget() {
    ...
    for method in ["claim", "private_claim", "claim_deposit"] {
...
fn agent_02_over_the_per_transaction_cap_is_denied() {
    ...
    assert!(e.authorize(&t, "1", 5 * PSY, "transfer").is_ok(), "exactly at the cap is allowed");
    let err = e.authorize(&t, "1", 6 * PSY, "transfer").unwrap_err().to_string();
...
fn agent_03_over_the_daily_cap_is_denied_and_says_what_is_left() {
    ...
    e.authorize(&t, "1", 10 * PSY, "transfer").unwrap();
    let err = e.authorize(&t, "1", 10 * PSY, "transfer").unwrap_err().to_string();
    ...
    assert!(e.authorize(&t, "1", 5 * PSY, "transfer").is_ok());
...
fn agent_04_over_the_thirty_day_cap_is_denied_even_with_daily_headroom() {
    ...
    e.authorize(&t, "1", 1_000, "transfer").unwrap();
    let err = e.authorize(&t, "1", 1_000, "transfer").unwrap_err().to_string();
    assert!(err.contains("30-day cap"), "the monthly cap must bind independently of the daily one: {err}");
    assert!(e.authorize(&t, "1", 500, "transfer").is_ok(), "exactly at the period cap is allowed");
}
...
fn agent_05_over_the_lifetime_budget_is_denied_and_never_resets() {
    ...
    e.authorize(&t, "1", 1_000, "transfer").unwrap();
    e.authorize(&t, "1", 500, "transfer").unwrap();
    ...
    assert!(e.authorize(&t2, "1", 1, "transfer").is_err(), "re-issuing a session must not top up the lifetime budget");
}
...
fn agent_06_an_unlisted_recipient_is_denied_without_leaking_the_allowlist() {
    ...
    assert!(e.authorize(&t, "Psy-00204800", 1, "transfer").is_ok(), "spelling must not decide the outcome");
    let err = e.authorize(&t, "31337", 1, "transfer").unwrap_err().to_string();
...
fn agent_06c_the_self_sentinel_cannot_be_forged_by_a_payee_named_self() {
    ...
    assert!(
        e.authorize_aliases(&t, &["31337", SELF_RECIPIENT], 1, "transfer").is_err(),
...
fn agent_07_a_revoked_session_stops_spending_immediately() {
    ...
    let (t2, _) = e.issue_session(&pid, 60, None).unwrap();
    assert!(e.authorize(&t2, "1", 1, "transfer").is_ok());
}
...
fn agent_07b_a_paused_policy_cannot_issue_a_fresh_session() {
    ...
    let (mut e, pid, t) = armed(wide(), None);
    assert!(e.pause(&pid));
    assert!(e.authorize(&t, "1", 1, "transfer").is_err(), "existing sessions stop spending");
    assert!(e.issue_session(&pid, 60, None).is_err(), "and no new session may be minted while paused");
    assert!(e.resume(&pid));
    assert!(e.authorize(&t, "1", 1, "transfer").is_ok(), "resume restores the existing session");
}
...
fn agent_09_spent_counters_survive_a_restart_but_sessions_do_not() {
    ...
    let (pid, token) = {
        ...
        e.authorize(&t, "204800", 1_000, "transfer").unwrap();
        (pid, t)
    };
    ...
    assert_eq!(d.allowed_methods, vec!["transfer".to_string()], "the method list survived");

    let (t2, _) = e2.issue_session(&pid, 60, None).unwrap();
    assert!(
        e2.authorize(&t2, "204800", 1_000, "transfer").is_err(),
        "a crash-loop must not re-grant the lifetime budget"
    );
    assert!(e2.authorize(&t2, "204800", 500, "transfer").is_ok());
    assert!(
        e2.authorize(&t2, "31337", 1, "transfer").is_err(),
...
fn agent_09c_the_on_disk_format_is_the_contract_a_restart_depends_on() {
    ...
    {
        let mut e = PolicyEngine::load_or_new(&dir);
        let pid = e.create_policy("matrix-agent", Limits { per_transaction: 7, per_day: 8, per_month: Some(9), total_budget: Some(10) }, Some(vec!["Psy-00000042".into()]), vec!["transfer".into()]);
        let (t, _) = e.issue_session(&pid, 60, None).unwrap();
        e.authorize(&t, "42", 5, "transfer").unwrap();
    }
...
fn agent_10_a_method_outside_the_list_is_denied_however_small_the_amount() {
    let mut e = PolicyEngine::new();
    let pid = e.create_policy("matrix-agent", wide(), None, vec!["transfer".into()]);
    let (t, _) = e.issue_session(&pid, 60, None).unwrap();
    assert!(e.authorize(&t, "1", 1, "transfer").is_ok());
    for denied in ["withdraw", "private_transfer", "x402_fetch", "deposit", "SIMPLE_TRANSFER", "transfer "] {
        assert!(
...
fn agent_13_a_refund_returns_headroom_without_ever_creating_new_budget() {
    ...
    let (mut e, pid, t) = armed(limits, None);
    let auth = e.authorize(&t, "1", 100, "transfer").unwrap();
    assert!(e.authorize(&t, "1", 1, "transfer").is_err(), "budget is exhausted");
    ...
    assert!(e.authorize(&t, "1", 1, "transfer").is_err(), "and it is exhausted again at exactly the cap");
}
...
fn agent_14_a_near_max_cap_must_not_silently_authorize_a_max_spend() {
    ...
    e.authorize(&t, "1", 1, "transfer").unwrap();

    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        e.authorize(&t, "1", u64::MAX, "transfer").is_ok()
    }));
    match outcome {
        Ok(true) => panic!(
