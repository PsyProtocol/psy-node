...
impl WalletSession {
    ...
    pub async fn get_eth_personal_secp_public_key(&self, private_key: QHashOut<F>) -> anyhow::Result<ZKPublicKeyInfo<F>> {
        self.wallet.get_eth_personal_secp_pk_info(private_key).await
    }
...
mod async_split_tests {
    ...
    async fn async_generate_prove_simple_mint() -> anyhow::Result<()> {
        ...
        let generated = simulated
            .generated
            .as_ref()
            .expect("simple_mint simulation must emit a generated trace");
        let tx_hash = simulated
            .metadata
            .tx_hash
            .as_ref()
            .expect("simple_mint simulation must emit tx_hash");
