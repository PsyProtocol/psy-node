...
use psy_client_data::dpn::sd_key::{SDKeyConfig, SDKeyTransactionInfo};
...
impl SDKeyCircuitGadget {
    ...
    pub fn add_virtual_to(builder: &mut CircuitBuilder<GF, D>, config: &SDKeyConfig, input_len: usize) -> Self {
    ...
    fn build_tx_introspection(builder: &mut CircuitBuilder<GF, D>, config: &SDKeyConfig) -> SDKeyTransactionIntrospectionGadget {
...
/// transactions exist, pad with `SDKeyTransactionInfo::default()` (all zeros)
...
pub fn compute_tx_stack_hash_padded(transaction_infos: &[SDKeyTransactionInfo<GF>], num_introspectable_transactions: u32) -> QHashOut<GF> {
    let n = num_introspectable_transactions as usize;
    let mut padded = transaction_infos.to_vec();
    padded.resize(n, SDKeyTransactionInfo::default());
