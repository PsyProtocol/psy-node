...
use psy_crypto::hash::tx_hash::{
    compute_deploy_contract_content_hash, compute_update_contract_content_hash,
};
use serde_json::json;
...
pub fn update_contract_content_hash(update_contract: &QBCUpdateContract<F>) -> anyhow::Result<String> {
...
pub async fn upload_update_contract_abi(
    network: &psy_config::NetworkConfigGoldilocks,
    update_contract: &QBCUpdateContract<F>,
    abi_json: &str,
) -> anyhow::Result<String> {
    if abi_json.len() > MAX_CONTRACT_ABI_JSON_BYTES {
        anyhow::bail!(
    ...
    let abi_value: serde_json::Value = serde_json::from_str(abi_json)?;
    let content_hash = update_contract_content_hash(update_contract)?;
    let services_url = resolve_services_url(network)?;
    let url = format!("{}/api/v1/contract/abi/pending-update", services_url);
    ...
    if !response.status().is_success() {
        ...
        let body = response.text().await.unwrap_or_default();
        anyhow::bail!(
            "failed to upload updated contract ABI to psy-services: HTTP {} {}",
            status,
            body
        );
    }
