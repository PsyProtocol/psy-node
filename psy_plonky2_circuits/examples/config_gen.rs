...
use parth_core::protocol::core_types::{QNetworkCircuitConstants, QNetworkTreeCircuitSpecificConstants, QNetworkTreeConstants};
...
cargo run --release --package psy_plonky2_circuits --example config_gen 
...
impl QNetworkTreeCircuitSpecificConstants for QPsyRegtestNetworkConstants {
...
fn run_gen_config<N: QNetworkCircuitConstants>(default_user_state_tree_root: QHashOut<GoldilocksField>) -> anyhow::Result<(String, String)> {
    ...
    println!("end_cap_fingerprint: {:?}", end_cap_fingerprint.to_u64x4());
