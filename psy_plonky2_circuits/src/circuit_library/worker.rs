...
pub async fn get_simple_proof_miner_worker_for_network<C: GenericConfig<D> + 'static, const D: usize>(
...
where     C::Hasher:AlgebraicHasher<C::F> + MerkleZeroHasher<HashOut<C::F>> + FieldQHasher<C::F, QHashOut<C::F>>, QHashOut<C::F>: Q256BitHash + QFHashBase<C::F>, C::F: QFelt64 + QRichField,
{
    ...
    job_fetcher.realm_api_url_manager.add_api_urls::<QHashOut<C::F>, QProvingJobDataID>(&config.realm_api_urls).await?;
