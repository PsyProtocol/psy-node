...
mod behavior_tests {
    ...
    fn qfelt_round_trip_default_and_hash_are_consistent() {
        let leaf = PQEDContractLeaf::<PF, Hash>::qp_rand_gen();
        let felts = leaf.to_qfelts();
        assert_eq!(felts.len(), 13);
        assert_eq!(PQEDContractLeaf::from_qfelts(&felts), leaf);
        assert_eq!(leaf.qfhash::<PoseidonHasher>(), PoseidonHasher::q_hash_many(&felts));
        assert_eq!(<PQEDContractLeaf<PF, Hash> as ToQFelts<PF>>::to_qfelts(&PQEDContractLeaf::default()).len(), 13);
        assert_eq!(leaf.self_qsize(), PQEDContractLeaf::<PF, Hash>::q_felt_size());
    }
    ...
    fn qfelt_decode_rejects_wrong_length() {
        ...
        let _ = <PQEDContractLeaf<PF, Hash> as ToQFelts<PF>>::from_qfelts(&felts);
    }
}
