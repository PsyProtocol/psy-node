...
impl<Hash: Copy> AggStateTransitionWithStats<Hash> {
    pub fn get_public_inputs_hash_no_tag_tree<H: FieldQHasher<F, Hash>, F: QFelt64>(&self, whitelist_root: Hash) -> Hash where Hash: HashTo4Felts<F>{
        let transition_hash = H::q_two_to_one(
            self.state_transition_start,
            self.state_transition_end,
        );
        let allowed_and_state_transition_hash = H::q_two_to_one(
            whitelist_root,
            transition_hash,
        );
...
pub struct AggStateTransitionInputV2<Hash> {
    ...
    pub right_proofs_generated_total: u64,
...
impl<Hash: Copy> WithDummyStateTransition<Hash> for AggStateTransitionInputV2<Hash> {
    fn get_dummy_value(state_root: Hash) -> Self {
        Self {
            ...
            right_proofs_generated_total: 0,
...
impl<Hash: QPGenRandom> QPGenRandom for AggStateTransitionInputV2<Hash> {
    fn qp_rand_gen() -> Self where Self: Sized {
        Self {
            ...
            right_proofs_generated_total: rand::random(),
...
impl<Hash: Q256BitHash> FallbackPsySerializeCanonical for AggStateTransitionInputV2<Hash> {
    ...
    fn fallback_pio_write_to_io<W: psy_io::Write>(&self, writer: &mut W) -> anyhow::Result<()> {
        ...
        writer.psy_write_u64(self.right_proofs_generated_total)?;
    ...
    fn fallback_pio_read_from_io<R: psy_io::Read>(reader: &mut R) -> anyhow::Result<Self> {
        ...
        let right_proofs_generated_total = reader.psy_read_u64()?;
