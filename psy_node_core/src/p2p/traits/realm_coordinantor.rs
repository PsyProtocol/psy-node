...
pub trait RealmCoordinatorClient<F, Hash> {
    ...
    async fn rc_submit_guta_proof(&self, input: GlobalUserTreeAggregatorHeaderWithTagValueAndJobType<F, Hash>, proof: Vec<u8>, realm_id: u64, proposal: Option<Vec<u8>>, certificate: Option<Vec<u8>>, finalize_binding: Vec<u8>) -> anyhow::Result<()>;
