...
use parth_core::{
    crypto::hash::{merkle_proof::{DeltaMerkleProofCore, MerkleProofCore}, tag_tree::TagTreeMerkleProof},
    data::{db::row::QDatabaseSingleIdTableRow, hash::{merkle_node_key::{SimpleMerkleNode, SimpleMerkleNodeKey}, merkle_store_key::{QMerkleStoreDoubleIdNode, QMerkleStoreSingleIdNode}}},
...
use parth_core::data::hash::merkle_store_key::{QMerkleStoreSingleIdKey, QMerkleStoreDoubleIdKey};
...
contract_code_definition_table: Arc<SingleIdTableIdentifier>,        
    ...
    async fn db_select_single_id_merkle_proof_max_checkpoint(
        ...
        db_helper_select_single_id_merkle_proof_max_checkpoint(
            &self.store,
            table,
            checkpoint_id,
            tree_id,
    ...
    async fn db_select_zero_id_merkle_proof_max_checkpoint(
        ...
        db_helper_select_zero_id_merkle_proof_max_checkpoint(
            &self.store,
            table,
            max_checkpoint_id,
            key,
    ...
    async fn db_select_zero_id_merkle_proof_max_checkpoint_to_root_level(
        ...
        db_helper_select_zero_id_merkle_proof_max_checkpoint_to_root_level(
            &self.store,
            table,
            max_checkpoint_id,
            root_level,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCheckpointTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCheckpointTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeUserRegistrationTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeUserRegistrationTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeGlobalUserTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    ...
    async fn global_user_tree_get_merkle_proof_sub_tree(&self, checkpoint_id: u64, root_level: u8, leaf_level: u8, leaf_index: u64) -> anyhow::Result<MerkleProofCore<N::QHash>> {
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeGlobalUserTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeUserContractTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn user_contract_tree_get_leaf_hash(&self, checkpoint_id: u64, user_id: u64, contract_id: u64) -> anyhow::Result<N::QHash> {
        ...
        self.store
            .db_select_single_id_merkle_node_max_checkpoint(&self.user_contract_tree_table, checkpoint_id, user_id, N::GLOBAL_CONTRACT_TREE_HEIGHT, key)
            .await
    }
    ...
    async fn user_contract_tree_get_merkle_proof(&self, checkpoint_id: u64, user_id: u64, contract_id: u64) -> anyhow::Result<MerkleProofCore<N::QHash>> {
        let key = SimpleMerkleNodeKey::new(N::GLOBAL_CONTRACT_TREE_HEIGHT, contract_id);
        self.db_select_single_id_merkle_proof_max_checkpoint(&self.user_contract_tree_table, checkpoint_id, user_id, N::GLOBAL_CONTRACT_TREE_HEIGHT, key)
            .await
    }
    ...
    async fn user_contract_tree_get_nodes(&self, checkpoint_id: u64, keys: &[QMerkleStoreSingleIdKey]) -> anyhow::Result<Vec<N::QHash>> {
        ...
        let simple_keys: Vec<SimpleMerkleNodeKey> = keys.iter().map(|k| SimpleMerkleNodeKey { level: k.level, index: k.index }).collect();
        self.store
            .db_select_many_single_id_merkle_nodes_max_checkpoint(&self.user_contract_tree_table, checkpoint_id, tree_id, N::GLOBAL_CONTRACT_TREE_HEIGHT, &simple_keys)
            .await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeUserContractTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn user_contract_tree_set_leaf_hash(&self, checkpoint_id: u64, user_id: u64, contract_id: u64, value: N::QHash) -> anyhow::Result<DeltaMerkleProofCore<N::QHash>> {
    ...
    async fn user_contract_tree_set_nodes(&self, checkpoint_id: u64, nodes: &[QMerkleStoreSingleIdNode<N::QHash>]) -> anyhow::Result<()> {
        ...
        let simple_nodes: Vec<SimpleMerkleNode<N::QHash>> = nodes.iter().map(|n| SimpleMerkleNode {
            key: SimpleMerkleNodeKey { level: n.key.level, index: n.key.index },
            value: n.value,
        }).collect();
        self.store
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeContractStateTreeTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn contract_state_tree_get_leaf_hash(&self, checkpoint_id: u64, user_id: u64, contract_id: u64, state_slot_id: u64) -> anyhow::Result<N::QHash> {
        let key = SimpleMerkleNodeKey::new(N::MAX_CONTRACT_STATE_TREE_HEIGHT, state_slot_id);
        self.store
            .db_select_double_id_merkle_node_max_checkpoint(&self.contract_state_tree_table, checkpoint_id, user_id, contract_id, N::MAX_CONTRACT_STATE_TREE_HEIGHT, key)
            .await
    }
    ...
    async fn contract_state_tree_get_merkle_proof(&self, checkpoint_id: u64, user_id: u64, contract_id: u64, state_slot_id: u64) -> anyhow::Result<MerkleProofCore<N::QHash>> {
        let key = SimpleMerkleNodeKey::new(N::MAX_CONTRACT_STATE_TREE_HEIGHT, state_slot_id);
        self.db_select_double_id_merkle_proof_max_checkpoint(&self.contract_state_tree_table, checkpoint_id, user_id, contract_id, N::MAX_CONTRACT_STATE_TREE_HEIGHT, &key)
            .await
    }
    ...
    async fn contract_state_tree_get_nodes(&self, checkpoint_id: u64, keys: &[QMerkleStoreDoubleIdKey]) -> anyhow::Result<Vec<N::QHash>> {
        ...
        let simple_keys: Vec<SimpleMerkleNodeKey> = keys.iter().map(|k| SimpleMerkleNodeKey { level: k.level, index: k.index }).collect();
        self.store
            .db_select_many_double_id_merkle_nodes_max_checkpoint(&self.contract_state_tree_table, checkpoint_id, tree_id, tree_sub_id, N::MAX_CONTRACT_STATE_TREE_HEIGHT, &simple_keys)
            .await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeContractStateTreeTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn contract_state_tree_set_leaf_hash(&self, checkpoint_id: u64, user_id: u64, contract_id: u64, value: N::QHash) -> anyhow::Result<DeltaMerkleProofCore<N::QHash>> {
    ...
    async fn contract_state_tree_set_top_tree_merkle_proof(&self, checkpoint_id: u64, merkle_proof: &MerkleProofCore<N::QHash>) -> anyhow::Result<()> {
        self.store
            .db_insert_one_single_checkpointed_object(
                &self.checkpointed_object_table,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeGlobalContractTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeGlobalContractTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeContractFunctionTreeDatabaseReader<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn contract_function_tree_get_leaf_hash(&self, checkpoint_id: u64, contract_id: u64, function_id: u64) -> anyhow::Result<N::QHash> {
        ...
        self.store
            .db_select_single_id_merkle_node_max_checkpoint(&self.contract_function_tree_table, checkpoint_id, contract_id, N::CONTRACT_FUNCTION_TREE_HEIGHT, key)
            .await
    }
    ...
    async fn contract_function_tree_get_merkle_proof(&self, checkpoint_id: u64, contract_id: u64, function_id: u64) -> anyhow::Result<MerkleProofCore<N::QHash>> {
        let key = SimpleMerkleNodeKey::new(N::CONTRACT_FUNCTION_TREE_HEIGHT, function_id);
        self.db_select_single_id_merkle_proof_max_checkpoint(&self.contract_function_tree_table, checkpoint_id, contract_id, N::CONTRACT_FUNCTION_TREE_HEIGHT, key)
            .await
    }
    ...
    async fn contract_function_tree_get_nodes(&self, checkpoint_id: u64, keys: &[QMerkleStoreSingleIdKey]) -> anyhow::Result<Vec<N::QHash>> {
        ...
        let simple_keys: Vec<SimpleMerkleNodeKey> = keys.iter().map(|k| SimpleMerkleNodeKey { level: k.level, index: k.index }).collect();
        self.store
            .db_select_many_single_id_merkle_nodes_max_checkpoint(&self.contract_function_tree_table, checkpoint_id, tree_id, N::CONTRACT_FUNCTION_TREE_HEIGHT, &simple_keys)
            .await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeContractFunctionTreeDatabaseWriter<N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn contract_function_tree_set_leaf_hash(&self, checkpoint_id: u64, contract_id: u64, function_id: u64, value: N::QHash) -> anyhow::Result<DeltaMerkleProofCore<N::QHash>> {
        let mut res = db_helper_single_id_merkle_node_simple_set_leaves_fast_serialize::<N::QHash, N::HasherBase, _, _>(
            &*self.store,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCheckpointObjectDatabaseReader<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    ...
    async fn get_unique_pending_id_for_checkpoint_id(&self, checkpoint_id: u64) -> anyhow::Result<Option<(u64, QCoreProcCheckpointUniqueId)>> {
        let pending_id = self.store.db_select_u64_value(&self.checkpoint_id_to_pending_id_table, checkpoint_id).await?;
        if let Some(pid) = pending_id {
            let uid = self.store.db_select_one_u128_value_by_u64(&self.pending_id_to_pending_proc_id_table, pid).await?;
            if let Some(u) = uid {
    ...
    async fn get_checkpoint_id_for_unique_pending_id(&self, unique_pending_id: u64) -> anyhow::Result<Option<u64>> {
        self.store.db_select_u64_value(&self.pending_id_to_checkpoint_id_table, unique_pending_id).await
    }
    ...
    async fn get_current_unique_pending_id(&self) -> anyhow::Result<(u64, QCoreProcCheckpointUniqueId)> {
        ...
        let uid = self.store.db_select_one_u128_value_by_u64(&self.pending_id_to_pending_proc_id_table, pending_id).await?
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCheckpointRealmSpecificDatabaseReader<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn get_top_global_user_rewards_tree_proof_to_realm_at_unique_pending_id(&self, unique_pending_id: u64) -> anyhow::Result<TagTreeMerkleProof<N::QHash>> {
        self.store
            .db_select_one_single_checkpointed_object_value::<TagTreeMerkleProof<N::QHash>>(&self.checkpointed_object_table, CHECKPOINTED_OBJECT_TABLE_OBJ_ID_REALM_ROOT_TO_GLOBAL_REWARDS_TAG_TREE_ROOT_PROOF, unique_pending_id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("Rewards tree proof not found for unique_pending_id {}", unique_pending_id))
    }

    async fn get_top_global_user_rewards_tree_proof_to_realm_at_checkpoint_id(&self, checkpoint_id: u64) -> anyhow::Result<TagTreeMerkleProof<N::QHash>> {
        self.store
            .db_select_one_single_checkpointed_object_value::<TagTreeMerkleProof<N::QHash>>(&self.checkpointed_object_table, CHECKPOINTED_OBJECT_TABLE_OBJ_ID_REALM_ROOT_TO_GLOBAL_REWARDS_TAG_TREE_ROOT_PROOF, checkpoint_id)
            .await?
    ...
    async fn get_top_global_user_tree_proof_to_realm_root_at_checkpoint_id(&self, checkpoint_id: u64) -> anyhow::Result<MerkleProofCore<N::QHash>> {
        self.store
            .db_select_one_single_checkpointed_object_value::<MerkleProofCore<N::QHash>>(&self.checkpointed_object_table, CHECKPOINTED_OBJECT_TABLE_OBJ_ID_REALM_ROOT_TO_GLOBAL_USER_TREE_ROOT_MERKLE_PROOF, checkpoint_id)
            .await?
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCheckpointObjectDatabaseWriter<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn inc_unique_pending_id(&self, amount: u64) -> anyhow::Result<(u64, QCoreProcCheckpointUniqueId)> {
        let new_pending_id = self.store.db_inc_counter(&self.u64_singleton_table, U64_SINGLETON_TABLE_OBJ_ID_PENDING_ID, amount as i64).await?;
        let unique_id = rand::random::<u128>();
        self.store.db_insert_u64_u128_mapping_pair(&self.pending_id_to_pending_proc_id_table, new_pending_id, unique_id).await?;
    ...
    async fn set_unique_pending_id_checkpoint_id_mapping(&self, unique_pending_id: u64, checkpoint_id: u64) -> anyhow::Result<()> {
        self.store.db_set_u64_value(&self.pending_id_to_checkpoint_id_table, unique_pending_id, checkpoint_id).await
    }
    ...
    async fn set_checkpoint_id_to_unique_pending_id_mapping(&self, checkpoint_id: u64, unique_pending_id: u64, unique_id_struct: &QCoreProcCheckpointUniqueId) -> anyhow::Result<()> {
        self.store.db_set_u64_value(&self.checkpoint_id_to_pending_id_table, checkpoint_id, unique_pending_id).await?;
        self.store.db_insert_u64_u128_mapping_pair(&self.pending_id_to_pending_proc_id_table, unique_pending_id, *unique_id_struct).await
    }

    async fn set_latest_checkpoint_id(&self, checkpoint_id: u64) -> anyhow::Result<()> {
        self.store.db_set_u64_value(&self.u64_singleton_table, U64_SINGLETON_TABLE_OBJ_ID_CHECKPOINT_ID, checkpoint_id).await
    }
    ...
    async fn set_checkpoint_root_hash_to_id_mapping(&self, checkpoint_root: N::QHash, checkpoint_id: u64) -> anyhow::Result<()> {
        self.store.db_insert_pair_ref(&self.checkpoint_root_to_checkpoint_id_table, &checkpoint_root, &checkpoint_id).await
    }
    ...
    async fn set_realm_rewards_tag_tree_top_proof_at_unique_pending_id(&self, unique_pending_id: u64, merkle_proof: &TagTreeMerkleProof<N::QHash>) -> anyhow::Result<()> {
        self.store.db_insert_one_single_checkpointed_object(&self.checkpointed_object_table, CHECKPOINTED_OBJECT_TABLE_OBJ_ID_REALM_ROOT_TO_GLOBAL_REWARDS_TAG_TREE_ROOT_PROOF, unique_pending_id, merkle_proof).await
    }

    async fn set_realm_rewards_tag_tree_top_proof_at_checkpoint_id(&self, checkpoint_id: u64, merkle_proof: &TagTreeMerkleProof<N::QHash>) -> anyhow::Result<()> {
        self.store.db_insert_one_single_checkpointed_object(&self.checkpointed_object_table, CHECKPOINTED_OBJECT_TABLE_OBJ_ID_REALM_ROOT_TO_GLOBAL_REWARDS_TAG_TREE_ROOT_PROOF, checkpoint_id, merkle_proof).await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreDatabaseUserStoreReader<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    ...
    async fn get_user_ids_for_public_key(&self, public_key: N::QHash, start_user_id: u64, count: usize) -> anyhow::Result<Vec<u64>> {
        self.store.db_select_value_u64_ids_for_hash(&self.public_key_hash_to_user_ids_table, public_key, count, start_user_id).await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreDatabaseUserStoreWriter<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    ...
    async fn set_user_leaves_ffs(&self, checkpoint_id: u64, data: &[u8]) -> anyhow::Result<()> {
        // Assume id at end or specific location, here using with_id_at_index, assume location = 0 for example
        let object_size = std::mem::size_of::<PQEDUserLeaf<N::F, N::QHash>>();
        let object_id_location = 0; // Assume id at offset 0
        self.store.db_insert_many_single_checkpointed_objects_at_checkpoint_ffs_with_id_at_index(&self.user_leaf_table, object_size, object_id_location, checkpoint_id, data).await
    }

    async fn set_zk_public_key(&self, checkpoint_id: u64, user_id: u64, public_key_info: &PZKPublicKeyInfo<N::QHash>) -> anyhow::Result<()> {
        self.store.db_insert_one_single_checkpointed_object(&self.user_public_key_table, user_id, checkpoint_id, public_key_info).await
    }
    ...
    async fn set_zk_public_keys_ffs(&self, checkpoint_id: u64, data: &[u8]) -> anyhow::Result<()> {
        ...
        let object_id_location = 0; // Assume
        self.store.db_insert_many_single_checkpointed_objects_at_checkpoint_ffs_clip_id_at_start(&self.user_public_key_table, object_size, checkpoint_id, data).await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreDatabaseBasicContractInfoStoreReader<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreDatabaseContractObjectStoreReader<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn get_contract_leaf(&self, checkpoint_id: u64, contract_id: u64) -> anyhow::Result<PQEDContractLeaf<N::F, N::QHash>> {
        self.store
            ...
            .await?
            .ok_or_else(|| anyhow::anyhow!("Contract leaf not found for contract_id {} at checkpoint_id {}", contract_id, checkpoint_id))
    }

    async fn get_contract_code_definition(&self, checkpoint_id: u64, contract_id: u64) -> anyhow::Result<ContractCodeDefinition> {
        self.store
            .db_select_one_single_checkpointed_object_value::<ContractCodeDefinition>(&self.contract_code_definition_table, contract_id, checkpoint_id)
            .await?
            .ok_or_else(|| anyhow::anyhow!("Contract code definition not found for contract_id {} at checkpoint_id {}", contract_id, checkpoint_id))
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreDatabaseContractObjectStoreWriter<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn set_contract_leaf(&self, checkpoint_id: u64, contract_id: u64, leaf: &PQEDContractLeaf<N::F, N::QHash>) -> anyhow::Result<()> {
        self.store.db_insert_one_single_checkpointed_object(&self.contract_leaf_table, contract_id, checkpoint_id, leaf).await
    }

    async fn set_contract_leaves_ffs(&self, checkpoint_id: u64, data: &[u8]) -> anyhow::Result<()> {
        self.store.db_insert_many_single_checkpointed_objects_at_checkpoint_ffs_clip_id_at_start(&self.contract_leaf_table, PSY_OBJECT_FFS_SIZE_CONTRACT_LEAF, checkpoint_id, data).await
    }

    async fn set_contract_code_definition(&self, checkpoint_id: u64, contract_id: u64, code_definition: &ContractCodeDefinition) -> anyhow::Result<()> {
        self.store.db_insert_one_single_checkpointed_object(&self.contract_code_definition_table, contract_id, checkpoint_id, code_definition).await
    }

    async fn set_many_contract_code_definitions(&self, checkpoint_id: u64, inserts: &[QDatabaseSingleIdTableRow<ContractCodeDefinition>]) -> anyhow::Result<()> {
        self.store.db_insert_many_single_checkpointed_object_rows(&self.contract_code_definition_table, inserts).await
    }
}
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreRewardsTagTreeStoreReader<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn rewards_tag_tree_get_root_at_unique_pending_id(&self, unique_pending_id: u64) -> anyhow::Result<N::QHash> {
        let root = self.store.db_get_tag_tree_root(&self.guta_reward_tag_tree_table, unique_pending_id).await?;
    ...
    async fn rewards_tag_tree_get_node_at_unique_pending_id(&self, unique_pending_id: u64, node: SimpleMerkleNodeKey) -> anyhow::Result<N::QHash> {
        self.store.db_get_tag_tree_node_value(&self.guta_reward_tag_tree_table, unique_pending_id, &node).await?
            .ok_or_else(|| anyhow::anyhow!("Node not found"))
    }

    async fn rewards_tag_tree_get_node_values_at_unique_pending_id(&self, unique_pending_id: u64, nodes: &[SimpleMerkleNodeKey]) -> anyhow::Result<Vec<Option<N::QHash>>> {
        self.store.db_get_tag_tree_node_values(&self.guta_reward_tag_tree_table, unique_pending_id, nodes).await
    }

    async fn rewards_tag_tree_get_node_tags_at_unique_pending_id(&self, unique_pending_id: u64, nodes: &[SimpleMerkleNodeKey]) -> anyhow::Result<Vec<Option<N::QHash>>> {
        self.store.db_get_tag_tree_node_tags(&self.guta_reward_tag_tree_table, unique_pending_id, nodes).await
    }

    async fn rewards_tag_tree_get_tag_tree_merkle_proof_at_unique_pending_id(&self, unique_pending_id: u64, nodes: &[SimpleMerkleNodeKey]) -> anyhow::Result<Vec<Option<N::QHash>>> {
        // The trait has get_tag_tree_merkle_proof_at_unique_pending_id, but param nodes, but return Vec<Option<Hash>>, perhaps typo, probably for proof
...
impl<
        ...
        S: CoreDatabaseStore<
            ...
            > + Send + Sync,
    >
    PsyNodeCoreRewardsTagTreeStoreWriter<N::F, N::QHash> for PsyUnifiedCoreDatabaseStore<
        N,
        BiDirectionalMappingTableIdentifier,
        BiDirectionalU64U128MappingTableIdentifier,
        U64TableIdentifier,
    ...
    >
{
    async fn rewards_tag_tree_set_node_tag(
        ...
        self.store.db_set_tag_tree_tag_value(&self.guta_reward_tag_tree_table, unique_pending_id, &key, &tag, &value).await
    }
}