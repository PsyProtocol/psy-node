...
#[derive(Debug, Clone)]
pub struct SimpleDashTagTreeStore<Hasher, Hash: Copy + PartialEq + Default> {
    nodes: DashMap<SimpleMerkleNodeKey, TagTreeStorageNode<Hash>>,
    height: u8,
    _hasher: PhantomData<Hasher>,
}

impl<Hasher: MerkleHasher<Hash>, Hash: Copy + PartialEq + Default>
    SimpleDashTagTreeStore<Hasher, Hash>
{
    ...
    pub fn set_node_split(&self, key: SimpleMerkleNodeKey, value: Hash, tag: Hash) {
        self.nodes
            .insert(key, TagTreeStorageNode { value, tag });
    }

    pub fn set_tag(&self, key: SimpleMerkleNodeKey, tag: Hash) {
    ...
    pub fn set_node(&self, key: SimpleMerkleNodeKey, value: TagTreeStorageNode<Hash>) {
...
mod tests {
    ...
    use parth_core::{crypto::hash::traits::MerkleHasher, data::hash::{hash256::Hash256, merkle_node_key::{SimpleMerkleNodeKey, generate_nca_tree_groups_v0, generate_nca_tree_groups_v1}}};
    
    use super::SimpleDashTagTreeStore;
    ...
    fn test_merkle_tree_sha256_small() {
        ...
        let leaf_6 = SimpleMerkleNodeKey::new(guta_height, 6);
        let leaves = vec![leaf_1, leaf_2, leaf_3, leaf_5, leaf_6];
        let e_group_levels = generate_nca_tree_groups_v0(&leaves, guta_height);
