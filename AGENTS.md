...
Before `make run-all`, restart, rollback validation, or a live E2E, read and follow both `docs/src/dev/devnet_lifecycle.md` and `docs/src/dev/devnet-launcher-reference.md`:
...
8. Use `make rollback-stop` to pause applications for offline rollback. Use `PURGE=0 make shutdown` to stop the stack while retaining paired Anvil/L2 state. Use `PURGE=1 make shutdown` then `make run-all` only for a fresh chain.
...
1. Lead with the conclusion. Omit filler and repetition. If a response exceeds ~300 lines without structure, it is too verbose.
2. Explain architecture and protocol flows with ASCII charts, not prose walls. Every multi-step flow gets a diagram.
3. Explain logic with pseudocode showing the core function and parameter names. Real implementation excerpts only when pseudocode cannot resolve ambiguity.
