...
10. Prefer one common verb plus one concrete object for function names. Use direct verbs such as `get`, `set`, `check`, `read`, `write`, `load`, `save`, `add`, `remove`, `create`, `update`, `apply`, and `build`; avoid multi-verb names, process jargon, and subjective lifecycle labels when one plain action states the behavior.
