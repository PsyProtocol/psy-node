...
use crate::hash::merkle::gadgets::{
    delta_merkle_proof::{DeltaMerkleProofGadget, DeltaMerkleProofGadgetOptionFlags}, dual_variable_height_delta_merkle_proof_index::VariableHeightMerkleProofIndexBitInfoGadget}
;
...
mod tests {
    ...
    impl<F: RichField> SimpleDualVariableHeightDeltaMerkleProofCircuitPublicInputsResult<F> {
        pub fn from_public_inputs(public_inputs: &[F], tree_height: usize, max_merkle_proof_height: usize) -> Self {
            ...
            let is_first_bit_outside_height = felts_to_bool_vec(&public_inputs[14 + tree_height..14 + 2 * tree_height]);
            println!("is_bit_not_within_height: {:?}", is_bit_not_within_height.iter().map(|x|if *x { 1 } else { 0 }).collect::<Vec<_>>());
            println!("is_first_bit_outside_height: {:?}", is_first_bit_outside_height.iter().map(|x|if *x { 1 } else { 0 }).collect::<Vec<_>>());
        ...
        pub fn validate_from_inputs(&self, left_proof: &DeltaMerkleProofCore<QHashOut<F>>, right_proof: &DeltaMerkleProofCore<QHashOut<F>>, tree_height: usize, _max_merkle_proof_height: usize) {
            ...
            for i in 0..self.is_bit_not_within_height.len() {
                ...
                if i == left_proof.siblings.len() {
                    assert_eq!(self.is_first_bit_outside_height[i], true, "is_first_bit_outside_height does not match at index {}", i);
                }else{
                    assert_eq!(self.is_first_bit_outside_height[i], false, "is_first_bit_outside_height does not match at index {}", i);
                }
            }
        }
    }
        ...
        C::Hasher: AlgebraicHasher<C::F>,
    {
        pub fn new(
            ...
            let public_inputs = [
                ...
                gadget.bit_info.is_first_bit_outside_height.iter().map(|b| b.target).collect::<Vec<_>>(),
            ].concat();
