...
use crate::hash::merkle::gadgets::bits::{QVariableHeightBitInfo, VariableHeightBitInfo};

#[derive(Debug, Clone)]
pub struct VariableHeightDeltaMerkleProofInSubTreeGadget {
...
impl VariableHeightDeltaMerkleProofInSubTreeGadget {
    pub fn add_virtual_to_full<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        Self::add_virtual_to_full_with_subtree_root_index::<H, F, D>(builder, max_merkle_proof_height, tree_height, input_height_target)
    }
    pub fn add_virtual_to_full_with_subtree_root_index<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        Self::add_virtual_to_full_with_subtree_root_index_known::<H, F, D>(
            builder,
    ...
    pub fn add_virtual_to_full_with_subtree_root_index_known<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        assert!(
            tree_height >= max_merkle_proof_height,
            "tree height must be at least as large as the max merkle proof height"
        );
        ...
        let (old_root, new_root, bit_info) = Self::compute_roots::<H, F, D>(builder, tree_height, index, old_value, new_value, &siblings, height);
    ...
    pub fn compute_roots<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        builder.range_check(index, tree_height);
        let index_bits = builder.split_le(index, tree_height);
        println!("🧮 index_bits: {:?}", index_bits.len());
    ...
    pub fn compute_root_bits<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        for (ind, (&sibling, bit)) in siblings.iter().zip(index_bits.iter()).enumerate() {
            is_bit_not_within_height.push(is_remaining_levels_zero);

            // in this case, we do not ensure the top bits are cleared, and instead handle that later
        ...
        let merkle_proof_max_height = siblings.len();
        let tree_height = index_bits.len();
        if tree_height > merkle_proof_max_height {
            let false_target = builder._false();
            let true_target = builder._true();

                is_bit_not_within_height
                    .extend(vec![true_target; tree_height - merkle_proof_max_height]);
                
            if merkle_proof_max_height != 0 {
                let is_first_unaccounted_bit_first_outside_height = builder.not(is_first_bit_outside_height.last().cloned().unwrap());
                is_first_bit_outside_height.push(is_first_unaccounted_bit_first_outside_height);

            }

            while is_first_bit_outside_height.len() < tree_height {
    ...
    pub fn set_witness_params<W: Witness<F>, F: RichField>(
        ...
        for i in siblings.len()..self.siblings.len() {
...
#[derive(Debug, Clone)]
pub struct VariableHeightDeltaMerkleProofInSubTree<F: RichField> {
...
impl<F: RichField> VariableHeightDeltaMerkleProofInSubTree<F> {
    pub fn new(max_merkle_proof_height: usize, tree_height: usize, has_witness_height: bool) -> Self {
        Self {
            ...
            index: F::ZERO,
            siblings: vec![QHashOut::default(); max_merkle_proof_height],
            height: F::ZERO,
            bit_info: QVariableHeightBitInfo {
                index_bits: vec![false; max_merkle_proof_height],
                is_bit_not_within_height: vec![false; max_merkle_proof_height],
                is_first_bit_outside_height: vec![false; max_merkle_proof_height],
            },
            max_merkle_proof_height,
            tree_height,
            has_witness_height,
    ...
    pub fn from_params(
        ...
        let mut proof = Self::new(max_merkle_proof_height, tree_height, input_height.is_none());
        ...
        for i in siblings.len()..max_merkle_proof_height {
    ...
    pub fn compute_roots(&self) -> (QHashOut<F>, QHashOut<F>, QVariableHeightBitInfo) {
        ...
        let index_bits = (0..self.tree_height).map(|i| (index >> i) & 1 == 1).collect::<Vec<_>>();
    ...
    fn compute_bit_info(&self, index_bits: &[bool], height: usize) -> QVariableHeightBitInfo {
        let mut is_bit_not_within_height = vec![false; self.tree_height];
        let mut is_first_bit_outside_height = vec![false; self.tree_height];
...
mod tests {
    ...
    use super::VariableHeightDeltaMerkleProofInSubTreeGadget;
    ...
    struct TestVariableHeightDeltaMerkleProofCircuit {
        pub variable_delta_merkle_proof_gadget: VariableHeightDeltaMerkleProofInSubTreeGadget,
        pub height: Target,
    ...
    impl TestVariableHeightDeltaMerkleProofCircuit {
        pub fn new(max_merkle_proof_height: usize, tree_height: usize) -> Self {
            ...
            let variable_delta_merkle_proof_gadget = VariableHeightDeltaMerkleProofInSubTreeGadget::add_virtual_to_full::<PoseidonHash, F, D>(
                &mut builder,
                max_merkle_proof_height,
                tree_height,
                Some(height),
    ...
    struct TestVariableHeightSubRootMerkleProofCircuit {
        pub variable_dmp_gadget_a: VariableHeightDeltaMerkleProofInSubTreeGadget,
        pub variable_dmp_gadget_b: VariableHeightDeltaMerkleProofInSubTreeGadget,
        pub level_a: Target,
        pub level_b: Target,
    ...
    impl TestVariableHeightSubRootMerkleProofCircuit {
        pub fn new(max_merkle_proof_height: usize, tree_height: usize) -> Self {
            ...
            let variable_dmp_gadget_a = VariableHeightDeltaMerkleProofInSubTreeGadget::add_virtual_to_full_with_subtree_root_index::<
                PoseidonHash,
                F,
                D,
            >(&mut builder, max_merkle_proof_height, tree_height, Some(height_a));

            let variable_dmp_gadget_b = VariableHeightDeltaMerkleProofInSubTreeGadget::add_virtual_to_full_with_subtree_root_index::<
                PoseidonHash,
                F,
                D,
            >(&mut builder, max_merkle_proof_height, tree_height, Some(height_b));
    ...
    fn test_variable_merkle_proof_sub_tree_circuit2() {
        let circuit = TestVariableHeightSubRootMerkleProofCircuit::new(32, 32);
        let height = 32;
    ...
    fn test_variable_delta_merkle_proof_circuit() {
        let circuit = TestVariableHeightDeltaMerkleProofCircuit::new(32, 32);
        let merkle_proofs = (1..32)
    ...
    fn test_variable_height_delta_merkle_proof_opt() -> anyhow::Result<()> {
        use super::{VariableHeightDeltaMerkleProofInSubTree, VariableHeightDeltaMerkleProofInSubTreeGadget};
        let max_merkle_proof_height = 4;
        let tree_height = 4;

        let index = 8;
        let old_value = QHashOut::<F>::rand();
        let new_value = QHashOut::<F>::rand();
        let siblings = (0..max_merkle_proof_height).map(|_| QHashOut::<F>::rand()).collect::<Vec<_>>();

        let plain_proof = VariableHeightDeltaMerkleProofInSubTree::from_params(
            index,
            old_value.clone(),
            new_value.clone(),
            &siblings,
        ...
        let variable_height_delta_merkle_proof_opt_gadget =
            VariableHeightDeltaMerkleProofInSubTreeGadget::add_virtual_to_full_with_subtree_root_index::<PoseidonHash, F, D>(
                &mut builder,
                max_merkle_proof_height,
                tree_height,
                None,
