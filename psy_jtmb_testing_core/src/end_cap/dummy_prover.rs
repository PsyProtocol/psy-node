...
) -> anyhow::Result<PsyUPSDummyProverHelper<N, HttpClient, DummyUPSStandardEndCapCircuit<C>>>
{
    let private_key = get_test_circuit_authority_key(network);
    let prover = DummyUPSStandardEndCapCircuit::<C>::new(&private_key);
    create_dummy_prover_helper::<N, DummyUPSStandardEndCapCircuit<C>>(api_url, prover)
}