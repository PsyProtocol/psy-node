...
use crate::{
    ...
    proving::{
        gadgets::guta::{
            ...
            verify_historical_root::verify_historical_root_proof_gt,
        },
...
#[derive(Debug, Clone)]
pub struct GUTAVerifyTwoGUTAUpgradeCheckpointCircuitV2<C: JTMBCircuitConfig> {
...
impl<C: JTMBCircuitConfig> QJTMBProofCircuitBase<C::Hash> for GUTAVerifyTwoGUTAUpgradeCheckpointCircuitV2<C> {
...
impl<C: JTMBCircuitConfig> GUTAVerifyTwoGUTAUpgradeCheckpointCircuitV2<C> {
    pub fn new(
        ...
        let circuit_type = ProvingJobCircuitType::GUTATwoGUTAWithCheckpointUpgrade;
    ...
    pub fn prove_base(
        ...
        input: &GUTAVerifyTwoGUTAUpgradeCheckpointCircuitInputV2<C::F, C::Hash>,
        ...
        // Upgrade Checkpoints
        let (hist_a, current_a) = verify_historical_root_proof_gt::<C>(&input.left_historical_checkpoint_merkle_proof)?;
        jtmb_connect_ref(&hist_a, &input.left_header.checkpoint_tree_root, "left historical checkpoint root mismatch")?;
        let mut a_header = input.left_header.clone();
        a_header.checkpoint_tree_root = current_a;

        let (hist_b, current_b) = verify_historical_root_proof_gt::<C>(&input.right_historical_checkpoint_merkle_proof)?;
...
impl<L: PsyJTMBCircuitInfoLibrary<C::Hash>, C: JTMBCircuitConfig> QJTMBProofCircuit<C, L> for GUTAVerifyTwoGUTAUpgradeCheckpointCircuitV2<C> {
    fn jtmb_prove_with_raw_proofs_and_ref_library(
        ...
        let witness = GUTAVerifyTwoGUTAUpgradeCheckpointCircuitInputV2::<C::F, C::Hash>::psy_ser_from_slice(&input.base.witness)?;
