...
pub const TMP_QUEUE_PREFIX: &str = "TMOQUV1";
pub const TMO_KV_STORE_PREFIX: &str = "TMOKSV1";
...
impl ProofStoreRedisAsync {
    ...
    async fn remove_from_set_generic_internal(&self, ns_key: &str, member: &[u8]) -> anyhow::Result<()> {
        let mut con = self.pool.get().await?;
    ...
    pub async fn add_to_set_u64_internal(&self, ns_key: &str, member: u64) -> anyhow::Result<()> {
        let mut con = self.pool.get().await?;
        let _: () = con.sadd(ns_key, member).await?;
        Ok(())
    }
    pub async fn remove_from_set_u64_internal(&self, ns_key: &str, member: u64) -> anyhow::Result<()> {
        let mut con = self.pool.get().await?;
        let _: () = con.srem(ns_key, member).await?;
        Ok(())
    }
    pub async fn get_set_u64_internal(&self, ns_key: &str) -> anyhow::Result<Vec<u64>> {
        let mut con = self.pool.get().await?;
