...
pub trait QPTempStoreTableType {
...
pub trait QPBasicStoreKVReader {
    ...
    async fn get_exact_object_key_bytes<K: QPDSerializableFixed + Sync>(&self, table_type: u32, key: &K) -> anyhow::Result<Vec<u8>> {
        self.get_exact_bytes(table_type, &key.to_bytes()?).await
    }
    async fn get_exact_object<K: QPDSerializableFixed + Sync, V: QPDSerializable>(&self, table_type: u32, key: &K) -> anyhow::Result<V> {
    ...
    async fn get_exact_object_or_none<K: QPDSerializableFixed + Sync, V: QPDSerializable>(&self, table_type: u32, key: &K) -> anyhow::Result<Option<V>> {
        let key_bytes = self.get_exact_object_key_bytes(table_type, key).await?;
        let value_bytes = self.get_exact_bytes(table_type, &key_bytes).await;
        match value_bytes {
            Ok(bytes) => {
    ...
    async fn get_exact_object_key_bytes_many<K: QPDSerializableFixed + Sync>(&self, table_type: u32, keys: &[K]) -> anyhow::Result<Vec<Vec<u8>>> {
    ...
    async fn get_exact_object_many<K: QPDSerializableFixed + Sync, V: QPDSerializable>(&self, table_type: u32, keys: &[K]) -> anyhow::Result<Vec<V>> {
        let key_bytes: Vec<Vec<u8>> = keys.iter().map(|k| k.to_bytes()).collect::<Result<_, _>>()?;
...
pub trait QPTempStoreKVU64Reader {
    ...
    async fn get_iu64_object_key_bytes<K: QPDSerializableFixed + Sync>(&self, table_type: u32, key: &K) -> anyhow::Result<u64> {
        self.get_iu64_generic(table_type, &key.to_bytes()?).await
    }
    async fn get_iu64_object<K: QPDSerializableFixed + Sync>(&self, table_type: u32, key: &K) -> anyhow::Result<u64> {
        self.get_iu64_generic(table_type, &key.to_bytes()?).await
    }
}
...
pub trait QPTempStoreKVU64Writer {
    ...
    async fn set_iu64_object_key_bytes<K: QPDSerializableFixed + Sync>(&self, table_type: u32, key: &K, value: u64) -> anyhow::Result<()> {
        self.set_iu64_generic(table_type, &key.to_bytes()?, value).await
    }
    async fn inc_iu64_object_key_bytes<K: QPDSerializableFixed + Sync>(&self, table_type: u32, key: &K, delta: i64) -> anyhow::Result<u64> {
