...
use crate::jobhq::nats::core::QueueJobData;

pub struct NatsJetStreamEmphemeralClient {
...
impl NatsJetStreamEmphemeralClient {
    ...
    pub async fn ensure_ephemeral_stream(&self) -> anyhow::Result<()> {
    ...
    pub async fn ensure_ephemeral_consumer(&self, subject: &str, durable_name: &str) -> anyhow::Result<()> {
    ...
    pub fn get_emphemeral_queue_subject(&self, realm_id: u64, realm_sub_id: u64, queue_type: u32, unique_topic: u128, task_group: u64) -> String {
        format!(
            "{}.r{:x}_{:x}.eq{}.u{:x}.g{}",
            self.base_namespace, realm_id, realm_sub_id, queue_type, unique_topic, task_group
        )
    }
    pub async fn ensure_ephemeral_stream_consumer(&self, subject: &str) -> anyhow::Result<()> {
        self.ensure_ephemeral_stream().await?;
        let durable_name = subject.replace('.', "_");
        self.ensure_ephemeral_consumer(subject, &durable_name).await
    }
    pub async fn ensure_ephemeral_stream_consumer_dn(&self, subject: &str) -> anyhow::Result<String> {
        self.ensure_ephemeral_stream().await?;
        let durable_name = subject.replace('.', "_");
        self.ensure_ephemeral_consumer(subject, &durable_name).await?;
        Ok(durable_name)
    }
    pub async fn push_messages_dq_ephemeral(&self, subject: &str, data: &[QueueJobData]) -> anyhow::Result<()> {
        self.ensure_ephemeral_stream_consumer(subject).await?;
    ...
    */
    ...
    pub async fn push_message_ephemeral(
        &self,
        realm_id: u64,
        realm_sub_id: u64,
        queue_type: u32,
        ...
        let subject = self.get_emphemeral_queue_subject(realm_id, realm_sub_id, queue_type, unique_topic, task_group);
        self.push_messages_dq_ephemeral(&subject, data).await
    }

    pub async fn dump_queue_ephemeral(
        &self,
        realm_id: u64,
        realm_sub_id: u64,
        queue_type: u32,
        ...
        let subject = self.get_emphemeral_queue_subject(realm_id, realm_sub_id, queue_type, unique_topic, task_group);
        self.dump_queue_dq_ephemeral(&subject, ack_messages, max_messages_per_batch, max_messages_total_to_dump)
            .await
    }

    pub async fn dump_queue_dq_ephemeral(
        &self,
        subject: &str,
        ack_messages: bool,
        max_messages_per_batch: usize,
    ...
    pub async fn get_message_if_exists_dq_ephemeral(&self, subject: &str) -> anyhow::Result<Option<QueueJobData>> {
           let durable_name = self.ensure_ephemeral_stream_consumer_dn(subject).await?;
    ...
    pub async fn get_message_if_exists_ephemeral(
        &self,
        realm_id: u64,
        realm_sub_id: u64,
        queue_type: u32,
        ...
        let subject = self.get_emphemeral_queue_subject(realm_id, realm_sub_id, queue_type, unique_topic, task_group);
        self.get_message_if_exists_dq_ephemeral(&subject).await
    }

    pub async fn report_message_completed_dq_ephemeral(&self, subject: &str, job_id: QueueJobData) -> anyhow::Result<()> {
    ...
    pub async fn report_message_completed_ephemeral(
        &self,
        realm_id: u64,
        realm_sub_id: u64,
        queue_type: u32,
        ...
        let subject = self.get_emphemeral_queue_subject(realm_id, realm_sub_id, queue_type, unique_topic, task_group);
        self.report_message_completed_dq_ephemeral(&subject, job_id).await
    }

    pub async fn wait_until_all_jobs_complete_or_timeout_dq_ephemeral(&self, subject: &str) -> anyhow::Result<()> {
        let durable_name = self.ensure_ephemeral_stream_consumer_dn(subject).await?;
    ...
    pub async fn wait_until_all_jobs_complete_or_timeout_ephemeral(
        &self,
        realm_id: u64,
        realm_sub_id: u64,
        queue_type: u32,
        ...
        let subject = self.get_emphemeral_queue_subject(realm_id, realm_sub_id, queue_type, unique_topic, task_group);
        self.wait_until_all_jobs_complete_or_timeout_dq_ephemeral(&subject).await
    }
}
