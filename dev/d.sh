...
start_worker() {
    build_if_needed $1
    ./target/release/psy_worker_cli worker --user 0 --network local-devnet --config ./psy_cli/example_node_configs/worker_coordinator.yml 2>&1 | tee logs/worker_coordinator_logs.txt
}
start_worker_realm() {
    build_if_needed_realm $1
...
start_dummy_prover() {
    build_if_needed $1
    ./target/release/psy_worker_cli dummy-prover --realm-url http://127.0.0.1:1338 --coordinator-url http://127.0.0.1:1337 --user 0 --network local-devnet --config ./psy_cli/example_node_configs/dummy_prover_1.yml 2>&1 | tee logs/dummy_prover_1_logs.txt
}
sub_worker() {
    start_worker $1
}
...
sub_dummy_prover() {
    ./target/release/psy_worker_cli dummy-end-cap-prover --coordinator-url http://127.0.0.1:1337 --url http://127.0.0.1:1338 --user 0 2>&1 | tee logs/dummy_end_cap_prover_logs.txt
}
