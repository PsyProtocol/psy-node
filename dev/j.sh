...
start_processor() {
    ...
    ./target/release/psy_node_cli start-coordinator-processor --proving-backend jtmb-poseidon-goldilocks --config ./psy_cli/example_node_configs/coordinator_processor_1.yaml 2>&1 | tee logs/coordinator_processor_1_logs.txt
}

start_edge() {
    build_if_needed $1
    ./target/release/psy_node_cli start-coordinator-edge --proving-backend jtmb-poseidon-goldilocks --config ./psy_cli/example_node_configs/coordinator_edge_1.yaml 2>&1 | tee logs/coordinator_edge_1_logs.txt
}

start_realm_edge() {
    build_if_needed_realm $1
    ./target/release/psy_node_cli start-realm-edge --proving-backend jtmb-poseidon-goldilocks  --config ./psy_cli/example_node_configs/realm_edge_1.yaml 2>&1 | tee logs/realm_edge_1_logs.txt
}
...
start_realm_processor() {
    ...
    # 2>&1 redirects stderr to stdout so errors are captured too
    # | tee writes to the file AND displays to console
    ./target/release/psy_node_cli start-realm-processor --proving-backend jtmb-poseidon-goldilocks --config ./psy_cli/example_node_configs/realm_processor_1.yaml 2>&1 | tee logs/realm_processor_1_logs.txt
}
...
start_worker_realm() {
    build_if_needed_realm $1
    ./target/release/psy_worker_cli worker --user 0 --network local-devnet --proving-backend jtmb-poseidon-goldilocks --config ./psy_cli/example_node_configs/worker_realm_1.yml 2>&1 | tee logs/worker_realm_1_logs.txt
}
start_dummy_prover() {
    build_if_needed $1
