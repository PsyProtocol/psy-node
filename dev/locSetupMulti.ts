    ...
    static async spawn(cmds: string[], options: {cwd?: string, stdOutVisitor?: ProcessLineVisitor, stdErrVisitor?: ProcessLineVisitor, allOutputVisitor?: ProcessLineVisitor, stdoutLogFile?: string, stderrLogFile?: string}): Promise<RunningProcess> {
        ...
        if (proc.stdout) {
            ...
            if (options.stdoutLogFile) {
                ...
                readableStream = logicBranch as any; // Cast generic stream
            ...
            (async () => {
                ...
                for await (const chunk of readableStream) {
                    // Note: We removed Bun.write(Bun.stdout, chunk) here to stop console spam
                    
                    // Parse logic is kept to ensure startup detection works
        ...
        if (proc.stderr) {
            ...
            (async () => {
                const decoder = new TextDecoder();
                for await (const chunk of readableStream) {
                    // Note: We removed Bun.write(Bun.stderr, chunk) here
...
class DevNetProcessManager {
    ...
    async setupProcesses(options: ProcessOptions): Promise<void> {
        ...
        this.scyllaProcess = this.track(await RunningProcess.spawnWithInitializationHint(
            ...
            { 
                cwd,
                stdoutLogFile: path.join(logsDir, "scylla_logs.txt"),
                stderrLogFile: path.join(logsDir, "scylla_errs.txt"),
            },
        ));
        ...
        this.coordinatorProcessor = this.track(await RunningProcess.spawnWithInitializationHint(
            ...
            { 
                cwd,
                stdoutLogFile: path.join(logsDir, "coordinator_processor_1_logs.txt"),
                stderrLogFile: path.join(logsDir, "coordinator_processor_1_errs.txt")
            },
        ));
        ...
        this.coordinatorEdge = this.track(await RunningProcess.spawnWithInitializationHint(
            ...
            { 
                cwd,
                stdoutLogFile: path.join(logsDir, "coordinator_edge_1_logs.txt"),
                stderrLogFile: path.join(logsDir, "coordinator_edge_1_errs.txt")
            },
        ));
        ...
        this.coordinatorWorker = this.track(await RunningProcess.spawnWithInitializationHint(
            ...
            { 
                cwd,
                stdoutLogFile: path.join(logsDir, "worker_1_logs.txt"),
                stderrLogFile: path.join(logsDir, "worker_1_errs.txt")
            },
        ));
        ...
        this.realmProcessor = this.track(await RunningProcess.spawnWithInitializationHint(
            ...
            { 
                cwd,
                stdoutLogFile: path.join(logsDir, "realm_processor_1_logs.txt"),
                stderrLogFile: path.join(logsDir, "realm_processor_1_errs.txt")
            },
        ));
        ...
        for (let i = 0; i < workerEdgeCount; i++) {
            let configPath = '';
            // If i==0, use the standard default config from the bash script logic
            if (i === 0) {
            ...
            const p = this.track(await RunningProcess.spawnWithInitializationHint(
                ...
                { 
                    cwd,
                    stdoutLogFile: path.join(logsDir, `${logPrefix}_logs.txt`),
                    stderrLogFile: path.join(logsDir, `${logPrefix}_errs.txt`)
                },
            ));
