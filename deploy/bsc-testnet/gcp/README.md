...
- The deployer/relayer keystore password and expected address.

The BSC configuration does not source the Sepolia profile. If the same hosts
need private site-level defaults, set `BSC_BASE_GCP_CONFIG` explicitly to a
network-neutral file; do not point it at either profile's `config.env`.
...
```bash
R2_SKIP_UPLOAD=1 \
  bash deploy/bsc-testnet/gcp/publish-wallet-r2.sh
```
...
```bash
bash deploy/bsc-testnet/gcp/prepare-sources.sh
BSC_PREFLIGHT_SKIP_RPC=1 \
  bash deploy/bsc-testnet/gcp/preflight.sh
```
...
CONFIRM_FULL_FRESH_DEPLOY=1 \
  bash deploy/bsc-testnet/gcp/deploy_all.sh
```
