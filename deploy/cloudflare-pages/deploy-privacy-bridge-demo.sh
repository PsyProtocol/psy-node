...
multichain_protocol_config_backup=""

patch_multichain_rpc_default() {
  local env_name="$1"
  local network="$2"
  local public_url count

  public_url="$(multichain_runtime_json | jq -er --arg network "$network" \
    '.chains[] | select(.network == $network) | "https://" + .public_rpc_domain')"
  count="$(grep -Ec "ENV\\.${env_name} \\|\\| 'https?://[^']+'" "$protocol_config_file" || true)"
  [ "$count" = "1" ] || {
    echo "expected one ${env_name} default in $protocol_config_file, found $count" >&2
    exit 1
  }
  sed -i -E \
    "s#ENV\\.${env_name} \\|\\| 'https?://[^']+'#ENV.${env_name} || '${public_url}'#" \
    "$protocol_config_file"
  grep -Fq "ENV.${env_name} || '${public_url}'" "$protocol_config_file" || {
    echo "failed to install public ${network} RPC URL in frontend protocol config" >&2
    exit 1
  }
}
...
cleanup_frontend_source() {
  ...
  if [ -n "$multichain_protocol_config_backup" ]; then
    cp "$multichain_protocol_config_backup" "$protocol_config_file"
    rm -f "$multichain_protocol_config_backup"
  fi
  if [ -n "$multichain_deployment_backup_dir" ]; then
    while IFS= read -r network; do
      ...
      if [ -f "$multichain_deployment_backup_dir/$network/existed" ]; then
        ...
        rm -f "$target_deployment"
      fi
    done < <(multichain_runtime_json | jq -r '.chains[].network')
    rm -rf "$multichain_deployment_backup_dir"
  elif [ "$deployment_existed" = "1" ]; then
...
if multichain_enabled; then
  ...
  multichain_protocol_config_backup="$(mktemp)"
  cp "$protocol_config_file" "$multichain_protocol_config_backup"
  patch_multichain_rpc_default SEPOLIA_RPC_URL sepolia
  patch_multichain_rpc_default BSC_TESTNET_RPC_URL bscTestnet
  patch_multichain_rpc_default BASE_SEPOLIA_RPC_URL baseSepolia
  export PSY_SKIP_CONFIG_SYNC=1
...
if multichain_enabled; then
  ...
  default_l1_rpc_url="https://$(jq -r '.public_rpc_domain' <<<"$primary_chain")"
  default_l1_explorer_url="$(jq -r '.explorer_url' <<<"$primary_chain")"
fi
