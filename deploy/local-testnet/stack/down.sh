...
stop_envio_compose() {
  local envio_compose="$PARTH_DIR/psy_cli/psy_relayer_cli/indexer/envio/generated/docker-compose.yaml"
  local compose_args=(down)

  [ -f "$envio_compose" ] || return 0
  if [ "$REMOVE_VOLUMES" = "1" ]; then
    compose_args+=(-v)
  fi

  echo "[local-staging] stopping Envio Docker dependencies"
  if docker compose version >/dev/null 2>&1; then
    docker compose -f "$envio_compose" "${compose_args[@]}"
  elif command -v docker-compose >/dev/null 2>&1; then
    docker-compose -f "$envio_compose" "${compose_args[@]}"
  else
    echo "[local-staging] docker compose is required to stop Envio" >&2
    return 1
  fi
}
