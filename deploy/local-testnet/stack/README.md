...
This lower-level entrypoint starts only the core stack. For the complete local
testnet, including Anvil, bridge contracts, Envio, relayer, Cloudflare Tunnel,
frontend auto deploy, and final health verification, use:
...
The checked-in `client_prover/config.json` already points the `localhost`
...
```text
client_prover/psy-privacy-bridge -> http://127.0.0.1:8088
client_prover/psy_explorer       -> http://127.0.0.1:8089
client_prover/psy_ide/frontend   -> http://127.0.0.1:8090
```

If dependencies are not installed, either install them manually in each frontend
directory or run:
...
`client_prover/psy-privacy-bridge/public/downloads`. To rebuild the wallet zip
from `/home/peter/git/bridge_zilong/psy-wallet` before publishing:
...
This is the lower-level local core chain and indexing stack. The top-level
`deploy/local-testnet/deploy-all.sh` composes it with the L1, bridge, shared
browser, frontend automation, and verification layers.
