...
is_usable_genesis_contracts() {
  local path="$1"
  [ -s "$path" ] || return 1
  json_or_zstdcat "$path" | jq -e 'type == "array" and length > 0' >/dev/null 2>&1
}
...
ensure_genesis_artifacts() {
  local genesis_contracts="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_contracts.json"
  ...
  is_usable_genesis_contracts "$genesis_contracts" || {
    echo "[local-staging] canonical psy-genesis contracts are missing or invalid: $genesis_contracts" >&2
    echo "[local-staging] initialize the psy-genesis submodule before starting" >&2
    exit 1
  }
  require_file "$LOCAL_STAGING_PSY_GENESIS_DIR/config.json"
  [ -d "$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi" ] || {
    echo "[local-staging] missing canonical genesis ABI directory" >&2
    exit 1
  }

  if [ ! -s "$GENESIS_PATH" ] || [ ! -s "$PRIVATE_KEYS_PATH" ]; then
...
verify_claim_deposit_artifacts() {
  ...
  local token_abi="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi/PsyTokenContract.json"
  local deposit_tree_abi="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi/PsyDepositTreeContract.json"
  local method_id
  local input_count
...
verify_genesis_runtime_abis() {
  local genesis_contracts="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_contracts.json"
  local genesis="$PARTH_DIR/genesis.json"
  local psy_token_abi="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi/PsyTokenContract.json"
  local withdrawal_tree_abi="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi/PsyWithdrawalTreeContract.json"
  local usdt_abi="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi/USDTTokenContract.json"
  local faucet_abi="$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_abi/PsyFaucetContract.json"
  local artifact
  local mismatches
...
main() {
  ...
  if [ "$LOCAL_STAGING_FAUCET_SPLIT_ONLY" = "1" ]; then
    if [ "$LOCAL_STAGING_BUILD" = "1" ]; then
      is_usable_genesis_contracts "$LOCAL_STAGING_PSY_GENESIS_DIR/genesis_contracts.json" || {
        echo "[local-staging] faucet-only build requires canonical psy-genesis contracts" >&2
        exit 1
      }
  ...
  if [ "$LOCAL_STAGING_START_PSY_SERVICES" = "1" ]; then
    ...
    local psy_services_env=(
      ...
      "PSY_NOSTR_RELAY_URL=ws://127.0.0.1:$LOCAL_NOSTR_PORT" \
