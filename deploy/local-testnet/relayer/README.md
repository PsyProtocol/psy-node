...
```bash
cd $HOME/git/bridge_zilong/pr-255-local-deploy
bash deploy/local-testnet/relayer/prepare-local-relayer.sh
```
...
```bash
bash deploy/local-testnet/relayer/install-systemd-user-service.sh
...
```bash
bash deploy/local-testnet/relayer/set-bridge-key.sh
```
