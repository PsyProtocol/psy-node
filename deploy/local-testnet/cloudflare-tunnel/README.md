...
The recommended complete entrypoint is:

```bash
bash deploy/local-testnet/deploy-all.sh
```

It invokes this layer, enables frontend auto deploy, verifies the finished
environment, and exits. Use `up.sh` directly only when debugging this layer:
...
```bash
PSY_WALLET_DIR="/home/peter/git/bridge_zilong/psy-wallet"
