...
build_relayer_if_requested() {
  ...
  CARGO_TARGET_DIR="${LOCAL_STAGING_TARGET_DIR:-$PARTH_DIR/target}" \
    PSY_NETWORK="${LOCAL_STAGING_CHAIN_CONFIG_NETWORK:-localhost}" cargo build --manifest-path "$PARTH_DIR/Cargo.toml" --release --bin psy_relayer_cli
}
...
snapshot_deployed_backend_abis() {
  [ "${LOCAL_CF_UPDATE_BACKEND_ABI_SNAPSHOT:-${LOCAL_STAGING_BUILD:-0}}" = "1" ] || {
