...
LOCAL_CF_AUTODEPLOY_BRANCH="${LOCAL_CF_AUTODEPLOY_BRANCH:-feat/improve-bridge-relayer}"
...
LOCAL_CF_AUTODEPLOY_PARTH_REPOSITORY="${LOCAL_CF_AUTODEPLOY_PARTH_REPOSITORY:-https://github.com/QEDProtocol/parth-generic-v1.git}"
...
LOCAL_CF_AUTODEPLOY_PARTH_DIR="${LOCAL_CF_AUTODEPLOY_PARTH_DIR:-$LOCAL_CF_AUTODEPLOY_SOURCE_ROOT/parth-generic-v1}"
...
write_current_source() {
  ...
  source_abi_sha="$(abi_manifest_hash "$LOCAL_CF_AUTODEPLOY_PARTH_DIR/genesis_abi")" || return $?
...
prepare_source_genesis_contracts() {
  local source_genesis_contracts="$1"
  local live_genesis_contracts="$LOCAL_CF_LIVE_PARTH_DIR/genesis_contracts.json"

  [ -s "$live_genesis_contracts" ] || {
    echo "[local-cf-autodeploy] missing deployed genesis contracts: $live_genesis_contracts" >&2
    return 1
  }
  [ ! -e "$source_genesis_contracts" ] && [ ! -L "$source_genesis_contracts" ] || {
    echo "[local-cf-autodeploy] clean source already contains genesis contracts: $source_genesis_contracts" >&2
    return 1
  }
  ln -s "$live_genesis_contracts" "$source_genesis_contracts"
}

build_and_publish() (
  local release_id release_dir_file release_dir source_deployment_file source_genesis_contracts source_genesis_link_created live_parth_dir live_state_dir
  ...
  source_deployment_file="$LOCAL_CF_AUTODEPLOY_PARTH_DIR/psy-contracts/deployments/localhost/deployed-contracts.json"
  source_genesis_contracts="$LOCAL_CF_AUTODEPLOY_PARTH_DIR/genesis_contracts.json"
  source_genesis_link_created=0
  ...
  cleanup_build_inputs() {
    ...
    if [ "$source_genesis_link_created" = "1" ]; then
      rm -f "$source_genesis_contracts"
    fi
  }
  trap cleanup_build_inputs EXIT
  prepare_source_deployment_file "$source_deployment_file" || return $?
  prepare_source_genesis_contracts "$source_genesis_contracts" || return $?
  source_genesis_link_created=1
