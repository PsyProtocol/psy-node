...
```text
/home/peter/git/bridge_zilong/local-testnet-latest-20260722/parth-generic-v1
```
...
```text
/home/peter/git/bridge_zilong/local-testnet-latest-20260722/frontend-autodeploy/parth-generic-v1
/home/peter/git/bridge_zilong/local-testnet-latest-20260722/frontend-autodeploy/psy-wallet
/home/peter/git/bridge_zilong/local-testnet-latest-20260722/frontend-autodeploy/psy-sdk
```

All three clean source checkouts track `feat/improve-bridge-relayer`. Do not use
them for manual development or commits.
...
Build and start the complete externally reachable environment, enable frontend
auto deploy, verify local and public health, and return after startup:

```bash
bash deploy/local-testnet/deploy-all.sh
```

For a fast restart that reuses the existing binaries and frontend artifacts:
...
Stop frontend automation, all managed processes, Envio, Cloudflare Tunnel, and
Docker services while retaining state and volumes:
...
```bash
bash deploy/local-testnet/stop-all.sh --volumes
```
...
/home/peter/git/bridge_zilong/cf_env
```
...
- Keep deployment-only changes on the deployment branch and runtime fixes on a
  branch targeting `feat/improve-bridge-relayer`.
