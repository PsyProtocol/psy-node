...
max_checkpoint_batch = ${RELAYER_MAX_CHECKPOINT_BATCH:-256}
...
This directory is generated from \`deploy/local-relayer/prepare-local-relayer.sh\`.
...
\`\`\`bash
bash "$PARTH_DIR/deploy/local-relayer/install-systemd-user-service.sh"
