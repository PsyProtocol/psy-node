...
write_relayer_config() {
  run_remote_script "$NAME" "$GCP_DIR/remote/write-relayer-config.sh" \
    ...
    "RELAYER_MAX_CHECKPOINT_BATCH=${RELAYER_MAX_CHECKPOINT_BATCH:-32}" \
...
upload_l1_deployments_artifacts() {
  if [ ! -d "$RELAYER_LOCAL_DEPLOYMENTS_DIR" ]; then
    echo "warning: local L1 deployments dir missing; relayer may miss Hardhat artifacts: $RELAYER_LOCAL_DEPLOYMENTS_DIR" >&2
    return
  fi

  if ! find "$RELAYER_LOCAL_DEPLOYMENTS_DIR" -maxdepth 1 -type f -name '*.json' | grep -q .; then
    echo "warning: no deployment JSON artifacts found in $RELAYER_LOCAL_DEPLOYMENTS_DIR" >&2
    return
  fi

  local archive="/tmp/parth-l1-deployments-${RELAYER_DEPLOYMENTS_NETWORK}.tar.gz"
  local remote_archive="/tmp/parth-l1-deployments-${RELAYER_DEPLOYMENTS_NETWORK}.tar.gz"
  (
    cd "$RELAYER_LOCAL_DEPLOYMENTS_DIR"
    tar -czf "$archive" ./*.json
  )

  echo "uploading L1 deployment artifacts: $RELAYER_LOCAL_DEPLOYMENTS_DIR/*.json -> ${NAME}:${remote_archive}"
  scp_to_remote "$NAME" "$archive" "$remote_archive"
  run_remote_command "$NAME" "sudo install -d -m 0755 '/opt/parth/current/psy-contracts/deployments/$RELAYER_DEPLOYMENTS_NETWORK' && sudo tar -xzf '$remote_archive' -C '/opt/parth/current/psy-contracts/deployments/$RELAYER_DEPLOYMENTS_NETWORK' && sudo chown -R parth:parth /opt/parth/current/psy-contracts/deployments && rm -f '$remote_archive'"
}
