...
L1_RPC_URL="${MULTICHAIN_CURRENT_L1_RPC_URL:-${L1_RPC_URL:-${ETH_RPC_URL:-http://${ANVIL_HOST}:${ANVIL_PORT}}}}"
...
L1_DEPLOYMENTS_NETWORK="${MULTICHAIN_CURRENT_L1_NETWORK:-${L1_DEPLOYMENTS_NETWORK:-${L1_NETWORK:-localhost}}}"
...
CHAIN_ID="${MULTICHAIN_CURRENT_CHAIN_ID:-${CHAIN_ID:-}}"
if [ -z "$CHAIN_ID" ]; then
  case "$L1_DEPLOYMENTS_NETWORK" in
    sepolia) CHAIN_ID="11155111" ;;
    bsc-testnet|bscTestnet) CHAIN_ID="97" ;;
    base-sepolia|baseSepolia) CHAIN_ID="84532" ;;
    *) CHAIN_ID="31337" ;;
  esac
fi
...
run_remote_script "$NAME" "$GCP_DIR/remote/deploy-l1-contracts.sh" \
  ...
  "L1_DEPLOY_WAIT_CONFIRMATIONS=${L1_DEPLOY_WAIT_CONFIRMATIONS:-1}" \
