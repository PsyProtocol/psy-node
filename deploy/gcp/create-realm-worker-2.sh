...
NAME="${REALM_WORKER_2_VM_NAME:-realm-worker-1}"
provision_vm "$NAME"
...
run_health_check "$NAME" "ports" \
  "HEALTHCHECK_PORTS=${REALM_WORKER_2_HEALTHCHECK_PORTS:-${REALM_WORKER_HEALTHCHECK_PORTS:-}}" \
  "HEALTHCHECK_HTTP_URLS=${REALM_WORKER_2_HEALTHCHECK_HTTP_URLS:-${REALM_WORKER_HEALTHCHECK_HTTP_URLS:-}}" \
