...
(
  unset PSY_NOSTR_ENABLED PSY_NOSTR_RELAY_URL PSY_NOSTR_RELAY_URLS PSY_NOSTR_LOOKBACK_SECONDS
...
(
  unset NOSTR_RELAY_URL PSY_NOSTR_RELAY_URL PSY_NOSTR_RELAY_URLS PSY_NOSTR_LOOKBACK_SECONDS
  PSY_NOSTR_ENABLED="false"
  resolve_psy_services_nostr_config
  [ "$PSY_NOSTR_ENABLED" = "0" ]
  [ -z "$PSY_NOSTR_RELAY_URL" ]
  [ -z "$PSY_NOSTR_RELAY_URLS" ]
)

(
  unset NOSTR_RELAY_URL PSY_NOSTR_ENABLED PSY_NOSTR_RELAY_URL PSY_NOSTR_LOOKBACK_SECONDS
  PSY_NOSTR_RELAY_URLS="wss://nostr-a.example.test/,wss://nostr-b.example.test/"
  resolve_psy_services_nostr_config
  [ "$PSY_NOSTR_ENABLED" = "1" ]
  [ "$PSY_NOSTR_RELAY_URL" = "wss://nostr-a.example.test/" ]
  [ "$PSY_NOSTR_RELAY_URLS" = "wss://nostr-a.example.test/,wss://nostr-b.example.test/" ]
)
...
grep -Fq '"PSY_NOSTR_RELAY_URL=$PSY_NOSTR_RELAY_URL"' "$GCP_DIR/deploy-psy-services.sh"
grep -Fq '"PSY_NOSTR_RELAY_URL=$PSY_NOSTR_RELAY_URL"' "$GCP_DIR/deploy-cp-ce-stack.sh"
grep -Fq 'upsert_if_set "$service_env" PSY_NOSTR_ENABLED PSY_NOSTR_ENABLED' \
  "$GCP_DIR/remote/deploy-parth-service.sh"
grep -Fq 'upsert_if_set "$service_env" PSY_NOSTR_RELAY_URL PSY_NOSTR_RELAY_URL' \
  "$GCP_DIR/remote/deploy-parth-service.sh"
