...
grep -F 'sudo systemctl restart \' "$DEPLOY_SCRIPT" >/dev/null || {
  echo "offsite worker deploy must restart existing services onto the new release" >&2
  exit 1
}

if grep -F 'sudo systemctl enable --now \' "$DEPLOY_SCRIPT" >/dev/null; then
