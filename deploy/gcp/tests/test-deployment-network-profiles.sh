...
[ "$(read_profile multi-chain)" = "sepolia|sepolia|11155111" ] || {
  echo "Multichain profile has an inconsistent primary network identity" >&2
  exit 1
}
...
for profile in ethereum-sepolia bsc-testnet multi-chain; do
...
echo "[ok] Sepolia, BSC, and multichain deployment profiles are internally consistent"
