    ...
    DummyUPSStateBuilder<F, Hash, Fetcher, Hasher>
{
    pub async fn new_init(
        ...
        let start_user_leaf = data_fetcher
            .df_get_user_leaf(checkpoint_id, user_id)
            .await?;

        Ok(Self {
            user_contract_tree: SimpleMemoryMerkleRecorderStore::new(
                user_contract_tree_height,
            ),
            data_fetcher,
            user_id,
            checkpoint_id,
    ...
    pub async fn populate_user_contract_tree(&mut self) -> anyhow::Result<()> {
        for contract_id in self.contract_state_tree_heights.keys() {
            ...
            let keys = siblings_keys.iter().map(|x| {
    ...
    pub async fn get_state_tree_height(&mut self, contract_id: u32) -> anyhow::Result<u8> {
        if self.contract_state_tree_heights.contains_key(&contract_id) {
            let height = *self.contract_state_tree_heights.get(&contract_id).ok_or_else(|| anyhow::anyhow!("No height found for contract"))?;
            return Ok(height);
        }
        let height = self
            .data_fetcher
            .df_get_contract_state_heights(u64::MAX-0xff, vec![contract_id as u64])
            .await?.pop().ok_or_else(|| anyhow::anyhow!("No height returned for contract"))?;
        ...
        println!("initialized tree with root {:?} and height {} for contract {}", tree.get_root(), height, contract_id);
        self.contract_state_trees.insert(contract_id, tree);
        Ok(height)
    }
    pub async fn get_merkle_proof_for_contract_from_fetcher(
        &mut self,
        contract_id: u32,
        slot_id: u64,
...
let height = self.get_state_tree_height(contract_id).await?;
        ...
        let mp:MerkleProofCore<Hash> = self.data_fetcher.df_get_contract_state_tree_merkle_proof(
                    self.checkpoint_id,
                    self.user_id,
                    contract_id as u64,
                    height,
        ...
        self.contract_state_trees
            .get_mut(&contract_id)
            .ok_or_else(|| anyhow::anyhow!("Contract tree not found"))?.injest_merkle_proof_into_nodes(&mp)?;
        Ok(mp)
    }
    pub async fn write_to_contract(&mut self, contract_id: u32, slot_id: u64, value_hash: Hash) -> anyhow::Result<()> {
        ...
        if slot_id >= (1u64 << state_height) {
            return Err(anyhow::anyhow!("Slot ID {} is out of bounds for contract {} with state tree height {}", slot_id, contract_id, state_height));
        }

        self.get_merkle_proof_for_contract_from_fetcher(contract_id, slot_id).await?;

        let dmp = self.contract_state_trees
            .get_mut(&contract_id)
            .ok_or_else(|| anyhow::anyhow!("Contract tree not found"))?.set_leaf(slot_id, value_hash);
        println!("old root for contract {}: {:?}", contract_id, dmp.old_root);
        let rt = self.contract_state_trees
            .get_mut(&contract_id)
            .ok_or_else(|| anyhow::anyhow!("Contract tree not found"))?.get_root();
        ...
        self.contract_state_dmps
            ...
            .push(dmp);
        for i in self.contract_state_dmps.get(&contract_id).ok_or_else(|| anyhow::anyhow!("No DMPs found for contract"))?.iter() {
    ...
    pub async fn read_from_contract(&mut self, contract_id: u32, slot_id: u64) -> anyhow::Result<Hash> {
        ...
        self.get_merkle_proof_for_contract_from_fetcher(contract_id, slot_id).await?;
        Ok(self.contract_state_trees.get(&contract_id).ok_or_else(|| anyhow::anyhow!("Contract tree not found"))?.get_leaf_value(slot_id))
    }
    ...
    pub async fn finalize_and_build(mut self) -> anyhow::Result<SubmitUserEndCapNonProofInput<F, Hash>> {
        ...
        for (contract_id, dmps) in self.contract_state_dmps.into_iter() {
            let contract_update = QEDContractStateUpdateHistory {
                user_contract_tree_update_proof: self.user_contract_tree.set_leaf(contract_id as u64, dmps.last().ok_or_else(|| anyhow::anyhow!("No DMPs found for contract"))?.new_root),
