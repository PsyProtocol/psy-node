...
use parth_core::{crypto::hash::merkle_proof::MerkleProofCore, data::hash::merkle_store_key::{QMerkleStoreDoubleIdKey, QMerkleStoreSingleIdKey}, protocol::core_types::QNetworkTypesConfig};
use psy_data::{proof_input::guta::end_cap_input::SubmitUserEndCapNonProofInput, v1::qdata::{public_key::PZKPublicKeyInfo, user::PQEDUserLeaf}};

use crate::api::{coordinator_fetcher::{PsyCoordinatorAPIFetcher, PsyCoordinatorFetcher, new_coordinator_fetcher_from_url}, data_fetcher::{PsyRealmAPIUserContractDataFetcher, PsyUserContractDataFetcher, new_contract_data_fetcher_from_url}};
...
#[derive(Clone)]
pub struct PsyDummyComboAPIFetcher<N: QNetworkTypesConfig + 'static, CC: PsyCoordinatorFetcher<N::QHash>, RC: PsyUserContractDataFetcher<N::F, N::QHash>> {
    pub cc: CC,
    pub rc: RC,
    _phantom_f: std::marker::PhantomData<N::F>,
}

impl<N: QNetworkTypesConfig + 'static, CC: PsyCoordinatorFetcher<N::QHash>, RC: PsyUserContractDataFetcher<N::F, N::QHash>> PsyDummyComboAPIFetcher<N, CC, RC> {
...
#[async_trait]
impl<N: QNetworkTypesConfig + 'static, CC: PsyCoordinatorFetcher<N::QHash> + Send + Sync + 'static, RC: PsyUserContractDataFetcher<N::F, N::QHash> + Send + Sync + 'static> PsyUserContractDataFetcher<N::F, N::QHash> for PsyDummyComboAPIFetcher<N, CC, RC> {
    ...
    async fn df_get_contract_state_tree_merkle_proof(&self, checkpoint_id: u64, user_id: u64, contract_id: u64, height: u8, slot_id: u64) -> anyhow::Result<MerkleProofCore<N::QHash>>{
        self.rc.df_get_contract_state_tree_merkle_proof(checkpoint_id, user_id, contract_id, height, slot_id).await
    }
...
#[async_trait]
impl<N: QNetworkTypesConfig + 'static, CC: PsyCoordinatorFetcher<N::QHash> + Send + Sync + 'static, RC: PsyUserContractDataFetcher<N::F, N::QHash> + Send + Sync + 'static> PsyCoordinatorFetcher<N::QHash> for PsyDummyComboAPIFetcher<N, CC, RC> {
...
pub fn new_combo_fetcher_from_urls<
    N: QNetworkTypesConfig + 'static,
>(coordinator_api_url: &str, realm_api_url: &str) -> anyhow::Result<PsyDummyComboAPIFetcher<N, PsyCoordinatorAPIFetcher<N, HttpClient>, PsyRealmAPIUserContractDataFetcher<N, HttpClient>>> {
