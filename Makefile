...
run-all: shutdown clean-db
	bun run dev/locSetupV4.ts --db-only --coordinator-only --realm-only --start-realm-id 0 --end-realm-id 3 --workers-only --coordinator-workers 1 --realm-workers 4 --env RUST_LOG=psy_node_common=debug

restart: shutdown
	bun run dev/locSetupV4.ts --coordinator-only --realm-only --start-realm-id 0 --end-realm-id 3 --workers-only --coordinator-workers 1 --realm-workers 4
