...
use parth_core::{crypto::hash::traits::MerkleZeroHasher, data::{db::table::QDatabaseTableRoutingKey, hash::{hash256::Hash256, merkle_store_key::{QMerkleStoreDoubleIdKey, QMerkleStoreDoubleIdNode}}}, pgoldilocks::{PGoldilocksHash, PoseidonHasher}, protocol::core_types::{QDBHashBase, QHashBase}, utils::QPGenRandom};
...
/* 
    ...
    group.bench_function(&format!("h256_insert_{}_QMerkleStoreDoubleIdNode_fast_serialized_data_fast_v5_grok_3_512", node_count), |b| {
        b.iter(|| {
            rt.block_on(double_id_merkle_table.set_double_id_merkle_nodes_batch_g_internal_fast_v5_grok_3_with_batch_size::<Hash>(&store.session, checkpoint_id_test_a, &fast_serialized_d[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..], 512)).unwrap();
        });
    });
    group.bench_function(&format!("h256_insert_{}_QMerkleStoreDoubleIdNode_fast_serialized_data_fast_v5_grok_3_256", node_count), |b| {
        b.iter(|| {
            rt.block_on(double_id_merkle_table.set_double_id_merkle_nodes_batch_g_internal_fast_v5_grok_3_with_batch_size::<Hash>(&store.session, checkpoint_id_test_a, &fast_serialized_d[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..], 256)).unwrap();
        });
    });
    group.bench_function(&format!("h256_insert_{}_QMerkleStoreDoubleIdNode_fast_serialized_data_fast_v5_grok_3_128", node_count), |b| {
        b.iter(|| {
            rt.block_on(double_id_merkle_table.set_double_id_merkle_nodes_batch_g_internal_fast_v5_grok_3_with_batch_size::<Hash>(&store.session, checkpoint_id_test_a, &fast_serialized_d[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..], 128)).unwrap();
        });
    });
    group.bench_function(&format!("h256_insert_{}_QMerkleStoreDoubleIdNode_fast_serialized_data_fast_v5_grok_3_128", node_count), |b| {
        b.iter(|| {
            rt.block_on(double_id_merkle_table.set_double_id_merkle_nodes_batch_g_internal_fast_v5_grok_3_with_batch_size::<Hash>(&store.session, checkpoint_id_test_a, &fast_serialized_d[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..], 64)).unwrap();
        });
    });
}
