...
REPORT_PATH="${WORKSPACE_DIR}/target/d04b6h23c4c4b4c1-sidecar-v13-rf3-report.json"
...
cd "${WORKSPACE_DIR}"
PSY_D04B6H23C4C4B4C1_RF3=1 \
PSY_D04B6H23C4C4B4C1_COMPOSE_FILE="${COMPOSE_FILE}" \
PSY_D04B6H23C4C4B4C1_REPORT_PATH="${REPORT_PATH}" \
cargo test -p psy_node_scylla \
  --features rf3-test-support \
  rollback::pending_queue_sidecar_schema_rf3::d04b6h23c4c4b4c1_sidecar_v13_lifecycle_rf3_gate \
  --lib -- --ignored --exact --nocapture

jq -e '
  .qualification == "H23C4C4B4C1_SIDECAR_V13_RF3_PASSED" and
  .replication_factor == 3 and
  .historical_schema_version == 12 and
  .current_schema_version == 13 and
  .target_tables == 20 and
  .lifecycle_tables == 1 and
  .control_targets == 16 and
  .data_targets == 4 and
  .expected_columns == 102 and
  .historical_schema_fingerprint == "466bf80f7e9f336a5191c2efdecf05129c96f6086f57c1afb14ce6cbe0aca7fb" and
  ...
  .v12_verified_rejected_for_v13 == true and
  .v12_payload_rejected_by_v13_decoder == true and
  .v13_slot_differs_from_v12 == true and
  .v13_deploy_idempotent == true and
  .different_current_rejected == true and
  .v12_lifecycle_preserved == true and
  .v12_representative_rows_preserved == true and
...
echo "D-04b6h23c4c4b4c1 sidecar v13 RF=3 report: ${REPORT_PATH}"
