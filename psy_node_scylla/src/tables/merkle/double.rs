...
impl ScyllaDoubleMerkleNodesPreparedStatements {
    pub async fn new_from_session(
        ...
        let delete_1_prepared = session.prepare(format!(
            "DELETE FROM {}.{} WHERE tree_id = ? AND tree_sub_id = ? AND level = ? AND node_index = ? AND checkpoint_id = ?",
            keyspace, table_name
        )).await?;
        let select_exact_1_prepared = session.prepare(format!(
            "SELECT checkpoint_id FROM {}.{} WHERE tree_id = ? AND tree_sub_id = ? AND level = ? AND node_index = ? AND checkpoint_id = ? LIMIT 1",
            keyspace, table_name
        )).await?;
...
impl ScyllaDoubleMerkleNodesPreparedStatements {
    pub async fn delete_many_tree_subtree_merkle_nodes(&self, session: &Session, keys: &[(u64, u64, u8, u64, u64)]) -> anyhow::Result<()> {
        for &(tree_id, tree_sub_id, level, node_index, checkpoint_id) in keys {
            session.execute_unpaged(
                &self.delete_1_prepared,
                (u64_to_i64_exact(tree_id), u64_to_i64_exact(tree_sub_id), u8_to_i8_exact(level), u64_to_i64_exact(node_index), convert_checkpoint_id_to_i64(checkpoint_id)),
            ).await?;
        }
        Ok(())
    }

    pub async fn contains_exact_tree_subtree_merkle_node(
        &self,
        session: &Session,
        key: (u64, u64, u8, u64, u64),
    ) -> anyhow::Result<bool> {
        let result = session.execute_unpaged(
            &self.select_exact_1_prepared,
            (
                u64_to_i64_exact(key.0),
                u64_to_i64_exact(key.1),
                u8_to_i8_exact(key.2),
                u64_to_i64_exact(key.3),
                convert_checkpoint_id_to_i64(key.4),
            ),
        ).await?;
