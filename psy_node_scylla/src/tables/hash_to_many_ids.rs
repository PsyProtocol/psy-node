...
use parth_core::{data::db::{hash_id_u64::read_hash256_refs_and_i64s_from_buffer, table::QDatabaseTableRoutingKey}, protocol::core_types::Q256BitHash};
...
use crate::{constants::INSERT_HASH_ID_TO_U64_VALUE_BATCH_SIZE, tables::traits::ScyllaStandardPreparedTableStatements, utils::{generate_batch_pre_prepared_statements, generate_batch_prepared_statement, i64_to_u64_exact, u64_to_i64_exact}};
    ...
    pub async fn create_table(session: Arc<Session>, keyspace: &str, table_name: &str, _table_key: QDatabaseTableRoutingKey) -> anyhow::Result<()> {
        session
            .query_unpaged( 
                ...
                ) WITH CLUSTERING ORDER BY (value_u64 ASC)", keyspace, table_name),
                &[],
            ).await?;
...
impl ScyllaHashToManyIdsTablePreparedStatements {
    ...
    pub async fn insert_one_hash_to_u64<Hash: DatabaseHashId>(
        &self, 
        session: &Session, 
        hash_id: Hash, 
        value: u64,
        ...
        session.execute_unpaged(&self.insert_1_prepared, (hash_bytes, u64_to_i64_exact(value))).await?;
        Ok(())
    }
    
    pub async fn insert_many_hash_to_u64s<Hash: DatabaseHashId>(
        &self, 
        session: &Session, 
        rows: &[(Hash, u64)],
    ) -> anyhow::Result<()> {
        ...
        // but Scylla binds BLOBs as Vec<u8> generally, though arrays can sometimes work.
        // Sticking to Vec<u8> for the hash for consistency with Scylla's binding logic for BLOB.
        ...
        for chunk in rows.chunks(INSERT_HASH_ID_TO_U64_VALUE_BATCH_SIZE) {
            ...
            let values: Vec<_> = chunk
                .iter()
                .map(|n| {
                    Ok((n.0.dhi_to_hash_bytes().to_vec(), u64_to_i64_exact(n.1)))
                })
    ...
    pub async fn set_hash_256_to_u64_pairs_from_fast_serialized_data(
        &self,
        session: &Session,
        data: &[u8],
    ) -> anyhow::Result<()> {
        if data.len() % 40 != 0 {
            anyhow::bail!("expected input bytes to be a multiple of 40 (hash + id), got a buffer of length {}",data.len());
        }
        ...
        stream::iter((0..num_of_batches))
            ...
            .try_for_each_concurrent(CONCURRENCY_LIMIT, |ind| {
                ...
                let group_size = if ind == num_of_batches - 1 {
                    count_remainder
                } else {
                    BATCH_SIZE
                };
    ...
    /// Selects up to `count` user IDs associated with the given hash, starting from `start_u64_value`.
    /// 
    /// # Pagination
    /// - To start, set `start_u64_value` to 0.
    /// - For subsequent pages, set `start_u64_value` to the largest `u64` value returned in the previous result **plus one**.
    ///   (This is required to skip the last returned element and continue to the next one).
    pub async fn select_value_u64_ids_for_hash<Hash: DatabaseHashId>(
        ...
        let res = session.execute_unpaged(
            &self.select_values_prepared, 
            (hash_bytes, start_u64_i64, count)
        ).await?;
        
        let rows_result = res.into_rows_result()?;

        // Rows contain tuples of (i64,) for the selected value_u64
        let rows: Vec<u64> = rows_result.rows::<(i64,)>()?.map(|x| {
            x.map_err(|e| anyhow::anyhow!("{:?}",e))
        }).collect::<anyhow::Result<Vec<_>>>()?.iter().map(|x| i64_to_u64_exact(x.0)).collect();
