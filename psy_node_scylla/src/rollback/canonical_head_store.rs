...
//!
//! Implements the revisioned CAS defined by
//! `psy_node_core::store::canonical_head` against a no-tablet keyspace, so a
//! head advance can only be published from an exact expected head.  The raw
//! `Session` stays private to this composition (design-r1 §2.1).
...
///
/// Current production keyspaces use `_no_tablet`. This wrapper prevents
/// accidentally pointing the canonical-head store at an
...
impl CanonicalHeadNoTabletKeyspace {
    pub fn try_new(name: impl Into<String>) -> Result<Self, CanonicalHeadStoreError> {
        let name = name.into();
        let keyspace = CqlKeyspaceName::try_new(name.clone())?;
        if !name.ends_with("_no_tablet") && !name.ends_with("_nt") {
            return Err(CanonicalHeadStoreError::InvalidNoTabletKeyspace(
...
impl CanonicalHeadReadBinding {
    pub fn try_from_network(network: NetworkId) -> Result<Self, CanonicalHeadStoreError> {
        Ok(Self {
...
impl ScyllaCanonicalHeadStore {
    ...
    ) -> Result<(), CanonicalHeadStoreError> {
    ...
    pub async fn prepare(
        session: Arc<Session>,
        no_tablet_keyspace: CanonicalHeadNoTabletKeyspace,
    ) -> Result<Self, CanonicalHeadStoreError> {
        let queries = CanonicalHeadQueries::new(&no_tablet_keyspace);
    ...
    pub async fn read<Hash: Q256BitHash>(
        &self,
        network: NetworkId,
    ) -> Result<CanonicalHeadReadState<Hash>, CanonicalHeadStoreError> {
    ...
    ) -> Result<CanonicalHeadWriteOutcome<Hash>, CanonicalHeadStoreError> {
            ...
            Ok(result) => {
                ...
                let current = match self.read(network).await? {
                    ...
                    CanonicalHeadReadState::Uninitialized => {
                        return Err(CanonicalHeadStoreError::CurrentMissingAfterLwt {
                            operation,
                            applied,
                        });
                    }
...
async fn prepare_lwt(
    ...
    cql: &str,
    contract: CanonicalHeadLwtContract,
) -> Result<PreparedStatement, CanonicalHeadStoreError> {
...
) -> Result<StoredCanonicalHead<Hash>, CanonicalHeadStoreError> {
...
pub fn decode_canonical_head_persisted_cells<Hash: Q256BitHash>(
    ...
    let chain_id = u32::try_from(network_chain_id)
        .map_err(|_| CanonicalHeadStoreError::NetworkChainIdOutOfRange(network_chain_id))?;
    ...
    if partition_network != requested_network {
        return Err(CanonicalHeadStoreError::SelectedPartitionMismatch {
            requested: requested_network,
            returned: partition_network,
        });
    }
    let revision = revision.ok_or(CanonicalHeadStoreError::MissingRevision)?;
    let canonical_ref = canonical_ref.ok_or(CanonicalHeadStoreError::MissingCanonicalPayload)?;
    let rollback_control =
        rollback_control.ok_or(CanonicalHeadStoreError::MissingRollbackControlPayload)?;
...
fn decode_lwt_applied(result: QueryResult) -> Result<bool, CanonicalHeadStoreError> {
    let rows = result.into_rows_result().map_err(cql_error)?;
    let applied_column = rows
        .column_specs()
        .get_by_name("[applied]")
        .ok_or(CanonicalHeadStoreError::MissingAppliedColumn)?;
    ...
    match row.columns.get(applied_column.0) {
        ...
        _ => Err(CanonicalHeadStoreError::InvalidAppliedColumn),
    }
}

fn cql_error(error: impl fmt::Display) -> CanonicalHeadStoreError {
    CanonicalHeadStoreError::Cql(error.to_string())
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum CanonicalHeadStoreError {
...
impl fmt::Display for CanonicalHeadStoreError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Cql(error) => write!(formatter, "canonical-head Scylla store failed: {error}"),
        }
    }
}

impl Error for CanonicalHeadStoreError {
...
impl From<InvalidCqlKeyspaceName> for CanonicalHeadStoreError {
    fn from(value: InvalidCqlKeyspaceName) -> Self {
        Self::InvalidKeyspace(value)
    }
}

impl From<CanonicalHeadModelError> for CanonicalHeadStoreError {
