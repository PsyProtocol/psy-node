//! Durable Scylla adapter for the per-authority commit timestamp allocator.
//!
//! One revisioned row per `(network, authority)`.  `seal_reservation` allocates
//! `max(high_water + 1, wall clock)`, so a commit timestamp is monotonic even if
//! the clock steps backwards, and `seal_completion` closes the lease.  Both are
//! LWT against the exact expected revision, so two writers cannot hold a lease
//! at once.
//!
//! This is the half of design-r1 §3.1 that carries no risk to a running node: it
//! allocates and persists a timestamp but changes no existing write.  Threading
//! `USING TIMESTAMP` through the table adapters is a separate step, because that
//! is the part that changes how a deployed node writes.
...
pub const AUTHORITY_COMMIT_TIMESTAMP_TABLE: &str =
    "authority_commit_timestamp";
...
impl AuthorityTimestampNoTabletKeyspace {
    ...
    ) -> Result<Self, AuthorityTimestampStoreError> {
        let name = name.into();
        let keyspace = CqlKeyspaceName::try_new(name.clone())?;
        if !name.ends_with("_no_tablet") && !name.ends_with("_nt") {
            return Err(AuthorityTimestampStoreError::InvalidNoTabletKeyspace(
...
impl AuthorityTimestampQueries {
    pub fn new(keyspace: &AuthorityTimestampNoTabletKeyspace) -> Self {
        let qualified = format!(
            "{}.{AUTHORITY_COMMIT_TIMESTAMP_TABLE}",
            keyspace.as_str()
        );
        let partition =
...
impl ScyllaAuthorityTimestampStore {
    ...
    ) -> Result<(), AuthorityTimestampStoreError> {
    ...
    pub async fn read(
        &self,
        key: AuthorityTimestampKey,
    ) -> Result<AuthorityTimestampReadState, AuthorityTimestampStoreError> {
        let result = self
            .session
    ...
    pub async fn read_observed(
        &self,
        key: AuthorityTimestampKey,
    ) -> Result<Option<ObservedAuthorityTimestampState>, AuthorityTimestampStoreError> {
    ...
    async fn finish_write(
        ...
        match execution {
            Ok(result) => {
                ...
                let current = match self.read(key).await? {
                    ...
                    AuthorityTimestampReadState::Uninitialized => {
                        return Err(
                            AuthorityTimestampStoreError::CurrentMissingAfterLwt {
                                operation,
                                applied,
                            },
                        );
            ...
            Err(error) => match self.read(key).await {
                ...
                Ok(_) => Err(AuthorityTimestampStoreError::IndeterminateWrite {
                    operation,
                    execute_error: error.to_string(),
                }),
                Err(read_error) => {
                    Err(AuthorityTimestampStoreError::IndeterminateReadFailed {
                        operation,
                        execute_error: error.to_string(),
...
async fn prepare_lwt(
    ...
    cql: &str,
    contract: AuthorityTimestampLwtContract,
) -> Result<PreparedStatement, AuthorityTimestampStoreError> {
...
) -> Result<StoredAuthorityTimestampState, AuthorityTimestampStoreError> {
...
pub fn decode_authority_timestamp_persisted_cells(
    ...
    if returned != expected {
        return Err(AuthorityTimestampStoreError::SelectedPartitionMismatch {
            expected: (
    ...
    let revision = revision.ok_or(AuthorityTimestampStoreError::MissingRevision)?;
    let state = state.ok_or(AuthorityTimestampStoreError::MissingStatePayload)?;
    StoredAuthorityTimestampState::decode_persisted(revision, state).map_err(Into::into)
}

fn decode_lwt_applied(
    result: QueryResult,
) -> Result<bool, AuthorityTimestampStoreError> {
    let rows = result.into_rows_result().map_err(cql_error)?;
    let applied_column = rows
        .column_specs()
        .get_by_name("[applied]")
        .ok_or(AuthorityTimestampStoreError::MissingAppliedColumn)?;
    ...
    match row.columns.get(applied_column.0) {
        ...
        _ => Err(AuthorityTimestampStoreError::InvalidAppliedColumn),
    }
}

fn cql_error(error: impl fmt::Display) -> AuthorityTimestampStoreError {
    AuthorityTimestampStoreError::Cql(error.to_string())
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum AuthorityTimestampStoreError {
...
impl fmt::Display for AuthorityTimestampStoreError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Cql(error) => {
                write!(formatter, "authority timestamp Scylla store failed: {error}")
            }
        }
    }
}

impl Error for AuthorityTimestampStoreError {
...
impl From<InvalidCqlKeyspaceName> for AuthorityTimestampStoreError {
    fn from(value: InvalidCqlKeyspaceName) -> Self {
        Self::InvalidKeyspace(value)
    }
}

impl From<AuthorityCommitModelError> for AuthorityTimestampStoreError {
...
mod tests {
    ...
    fn no_tablet_keyspace_is_mandatory() {
        ...
        assert!(matches!(
            ...
            Err(AuthorityTimestampStoreError::InvalidNoTabletKeyspace(_))
        ));
    }
    ...
    fn persisted_row_checks_partition_and_nullable_cells() {
        ...
        assert!(matches!(
            ...
            Err(AuthorityTimestampStoreError::SelectedPartitionMismatch { .. })
        ));
        assert_eq!(
            decode_authority_timestamp_persisted_cells(
                key(), 1337, 2, 7, 2, None, Some(&bytes)
            ),
            Err(AuthorityTimestampStoreError::MissingRevision)
        );
        assert_eq!(
            decode_authority_timestamp_persisted_cells(
                key(),
            ...
            Err(AuthorityTimestampStoreError::MissingStatePayload)
        );
    }
    ...
    // The spike guarded "created by the Coordinator setup, not the generic one"
    // with a source-text assertion over the setup files, which design-r1 §11.5
    // forbids.  In R1 the property is structural instead: the
